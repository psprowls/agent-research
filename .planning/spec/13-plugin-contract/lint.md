...
- **Companion module (semantic pass 2):** `vault_io.graph_analyzer.main`
  - This module already exists in vault-io. `lint_wiki.main` orchestrates both passes internally; `graph_analyzer` is invoked by `lint_wiki` as part of pass 2, not as a separate shim invocation.
...
| `### Pass 1 — Mechanical (scripts)` | Verbatim except script path rename (`scripts/lint_wiki.py` stays) and module reference rename (`lattice_wiki_core` → `vault_io`). |
...
| `skills/lattice-wiki/scripts/lint_wiki.py` | `skills/graph-wiki/scripts/lint_wiki.py` | Rename directory; rewrite imports (`lattice_wiki_core.lint_wiki` → `vault_io.lint_wiki`); bedrock branch shells to `code-wiki-agent lint`. |
...
**Behavior change #2 — `vault_io.lint_wiki` prerequisite port:**
As of Phase 13, `vault_io.lint_wiki` does not exist in this repo. The upstream module `lattice_wiki_core/lint_wiki.py` (~508 LOC) must be ported into `packages/vault-io/src/vault_io/lint_wiki.py` as Phase 14 Plan 1 before the shim can be activated. This follows VP-01..VP-04: verbatim port with brand rename (`lattice_wiki_core` → `vault_io`, `lattice-wiki` → `graph-wiki` in prose), same `main()` entry point and CLI argparse shape, same test surface (mirror upstream test structure), cleared by Phase 12's `scripts/check-brand.sh` grep gate (VP-03). No other behavior changes: mechanical pass 1 (existing `vault_io.lint/` rule modules) and semantic pass 2 (`vault_io.graph_analyzer`) are already present in vault-io and preserved.
