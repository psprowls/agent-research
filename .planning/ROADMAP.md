...
| 26. plugin-prompt-source-mirror-sync | 0/4 | Planned   | - |
...
**Goal:** Eliminate `packages/prompt-sources/` from the repository — re-point every `Source:` provenance comment and every divergence-rubric `source_anchor=` to the canonical surface (`plugins/graph-wiki/...`, the `workspace_io` runtime template, or a new agent-local sources tree for Bedrock-only roles), then delete the `packages/prompt-sources/` tree. After this phase, every anchor pointer references exactly one canonical surface and a brand-gate prevents reintroduction.
**Requirements**: D-01..D-13 (CONTEXT.md locked decisions; no REQ-IDs mapped — Phase 26 was a follow-on phase added after the v1.4 milestone audit)
**Depends on:** Phase 25
**Plans:** 4 plans
...
*Last updated: 2026-05-21 — Phase 26 planned (4 plans, 4 waves).*
