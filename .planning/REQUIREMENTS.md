...
- [ ] **LIBTOOLS-01** — `graph_tools.py` exists at `agents/graph-wiki-agent/src/graph_wiki_agent/graph_tools.py` exposing ≤5 `@tool`-decorated callables wrapping `graph_io.queries.*`
- [ ] **LIBTOOLS-02** — All tools declare `-> str` return type and serialize results via `graph_io.cli._format.render(records, fmt="human")`; row results hard-capped at 50 with explicit truncation notice
- [ ] **LIBTOOLS-03** — `build_graph_tools(conn)` factory accepts an open `graph_io.store` connection via closure; single connection opened at command entry, shared by all tool callables, closed in `finally`
