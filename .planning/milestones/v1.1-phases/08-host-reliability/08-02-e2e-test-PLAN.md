...
must_haves:
  truths:
    ...
    - "A single end-to-end integration test launches code-wiki-mcp as a stdio subprocess via uv run --package code-wiki-agent code-wiki-mcp and drives JSON-RPC over stdin/stdout (DACLI-01)."
    ...
    - "The test is gated by CODE_WIKI_RUN_INTEGRATION=1 because 4/6 tools call real Bedrock; the gate matches v1.0 integration-test conventions (DACLI-03)."
  artifacts:
    ...
    - path: "agents/code-wiki-agent/tests/unit/test_wiki_scan_input.py"
    ...
    - path: "agents/code-wiki-agent/tests/integration/test_mcp_e2e.py"
  ...
  key_links:
    ...
    - from: "test_mcp_e2e.py"
      to: "code-wiki-mcp subprocess"
      via: "subprocess.Popen(['uv', 'run', '--package', 'code-wiki-agent', 'code-wiki-mcp'])"
      pattern: "code-wiki-mcp"
...
<objective>
Add the missing `repo_path` field to `WikiScanInput` (required precondition — without it the E2E test would scan the live deep-agents workspace per RESEARCH.md Pitfall 4), then write a single sequential E2E integration test that launches `code-wiki-mcp` as a stdio subprocess and exercises all six shipped MCP tools against a fresh `tmp_path` vault.

Purpose: closes DACLI-01 (subprocess launch from spec-conformant host), DACLI-02 (all 6 tools exercised with non-error outcomes), and DACLI-03 (`CODE_WIKI_RUN_INTEGRATION=1` opt-in gate). This is the v1.1 proof that every shipped tool round-trips correctly through the real FastMCP stdio transport — not just unit-tested in isolation.
...
- `run_scan` (from `code_wiki_agent.commands.scan`) already accepts `repo_path: Path | None = None` (RESEARCH.md confirms — the override exists; only the MCP-layer plumbing is missing).
...
From agents/code-wiki-agent/tests/integration/test_mcp_stdio.py (PATTERNS.md lines 99-204):
- `_run_server(payload_objs)` helper at lines 63-78: spawns subprocess via `["uv", "run", "--package", "code-wiki-agent", "code-wiki-mcp"]`, feeds JSON-RPC over stdin, returns `(stdout, stderr)` decoded strings. 15s communicate timeout. Copy verbatim.
...
From agents/code-wiki-agent/tests/conftest.py lines 19-23:
- `INTEGRATION_GATE = pytest.mark.skipif(not os.environ.get("CODE_WIKI_RUN_INTEGRATION"), reason="Set CODE_WIKI_RUN_INTEGRATION=1 to run real Bedrock invocations")` — canonical definition. Import via `from tests.conftest import INTEGRATION_GATE` OR redefine locally (PATTERNS.md notes either is fine).

From agents/code-wiki-agent/tests/unit/test_mcp_new_tools.py (PATTERNS.md lines 234-263):
...
- Import-inside-test pattern (lines 31-37): `from code_wiki_mcp.server import WikiScanInput; inp = WikiScanInput(); assert inp.X == expected`.
...
<task type="auto" tdd="true">
  ...
  <files>agents/code-wiki-agent/src/code_wiki_mcp/server.py, agents/code-wiki-agent/tests/unit/test_wiki_scan_input.py</files>
  <behavior>
  ...
  <action>
**Part A — modify `agents/code-wiki-agent/src/code_wiki_mcp/server.py`** (PATTERNS.md §"agents/code-wiki-agent/src/code_wiki_mcp/server.py" lines 396-446):
...
**Part B — create `agents/code-wiki-agent/tests/unit/test_wiki_scan_input.py`** (PATTERNS.md §"agents/code-wiki-agent/tests/unit/test_wiki_scan_input.py" lines 230-287):
...
Do NOT add an `INTEGRATION_GATE` — these are pure schema tests, no Bedrock.
  ...
  <verify>
    <automated>uv run --package code-wiki-agent pytest agents/code-wiki-agent/tests/unit/test_wiki_scan_input.py -x -v</automated>
    Plus regression check: `uv run --package code-wiki-agent pytest agents/code-wiki-agent/tests/unit/ -x` (all unit tests including existing `test_mcp_new_tools.py` still pass).
  </verify>
  <done>
...
<task type="auto" tdd="true">
  ...
  <files>agents/code-wiki-agent/tests/integration/test_mcp_e2e.py</files>
  <behavior>
    ...
    - Launches `code-wiki-mcp` via the existing `_run_server` helper (copied from `test_mcp_stdio.py`) — single subprocess, single stdin pipeline.
    ...
    - `wiki_init` is the FIRST tool call (creates `.code-wiki/`); subsequent tools operate on the same vault (D-12 — fresh tmp_path, init + inline seed).
    - No `notifications/cancelled` in this test — cancel is Plan 01's scope.
  </behavior>
  <action>
Create `agents/code-wiki-agent/tests/integration/test_mcp_e2e.py` following PATTERNS.md §"agents/code-wiki-agent/tests/integration/test_mcp_e2e.py" lines 93-227.
...
- If `wiki_query` requires a built bm25 index that `wiki_init` does not produce, either (a) call `wiki_ingest` first against a seed page that builds the index, or (b) extend the seed helper to write `.code-wiki/index/` artifacts that `run_query` can read. Resolve by reading `commands/query.py` once before writing the test.
...
Do NOT wait for a JSON-RPC response on the `notifications/initialized` notification — notifications have no response per the MCP spec (Pitfall 5 in RESEARCH.md is for the cancel case, but the no-response-for-notifications rule applies universally).
  ...
  <verify>
    <automated>CODE_WIKI_RUN_INTEGRATION=1 uv run --package code-wiki-agent pytest agents/code-wiki-agent/tests/integration/test_mcp_e2e.py -x -v</automated>
    Skip-when-ungated check: `uv run --package code-wiki-agent pytest agents/code-wiki-agent/tests/integration/test_mcp_e2e.py -x -v` (must show 1 skipped, 0 failed).
  </verify>
  <done>
    ...
    `CODE_WIKI_RUN_INTEGRATION=1` run completes within ~180s and all six tools return non-error responses.
...
This phase introduces no new trust boundaries. The MCP stdio transport, subprocess spawn surface, and FastMCP tool handlers are all pre-existing surfaces with Phase 1–5 ASVS L1 coverage. The new `repo_path` field is a planner-controlled override consumed only by `run_scan` (already pathlib-resolved per the existing `vault_path` pattern). The E2E test runs only when the developer explicitly opts in via `CODE_WIKI_RUN_INTEGRATION=1`.
...
| T-08-02-N2 | (none) | subprocess spawn in test harness | accept | Test-only surface; gated by `CODE_WIKI_RUN_INTEGRATION=1`; no production code path invokes `subprocess.Popen` (RESEARCH.md §"Security Domain" — "The subprocess harness is test-only"). |
...
uv run --package code-wiki-agent pytest agents/code-wiki-agent/tests/unit/test_wiki_scan_input.py -x -v

# Ungated full suite — E2E test must SKIP, no failures
uv run --package code-wiki-agent pytest -x

# Gated integration suite — E2E test must PASS
CODE_WIKI_RUN_INTEGRATION=1 uv run --package code-wiki-agent pytest -x
```
...
- `test_mcp_e2e.py` launches the real `code-wiki-mcp` subprocess, drives JSON-RPC over stdin/stdout, exercises all six tools sequentially, asserts non-error for each, and respects the `CODE_WIKI_RUN_INTEGRATION=1` gate.
- DACLI-01, DACLI-02, DACLI-03 requirement IDs satisfied (VALIDATION.md rows 8-02-01 through 8-02-03).
- Total run time under `CODE_WIKI_RUN_INTEGRATION=1`: ≤180s (VALIDATION.md estimate).
