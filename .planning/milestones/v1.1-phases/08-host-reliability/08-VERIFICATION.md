...
| 4 | The cancel test runs unconditionally (no gate) and consumes zero Bedrock cost — make_llm is monkeypatched to a slow stub | VERIFIED | test_mcp_cancel.py has no INTEGRATION_GATE. `monkeypatch.setattr("code_wiki_agent.commands.query.make_llm", ...)` targets the importer's namespace (CR-01 fix confirmed at line 86). Test passes in 0.41s. |
...
| SC#2 | "automated cancel test ... runs under the standard opt-in gate" | Cancel test runs unconditionally — no CODE_WIKI_RUN_INTEGRATION=1 gate. | PLAN intentionally omitted gate because test uses stub LLM (zero Bedrock cost). Gate exists to prevent accidental cost; not applicable to cost-free tests. |
...
| `agents/code-wiki-agent/tests/integration/test_mcp_cancel.py` | Direct-asyncio cancel-mid-fan-out test | VERIFIED | Exists. 169 lines. `test_cancel_mid_fan_out` async function. Correct patch target (`code_wiki_agent.commands.query.make_llm`). All 4 trace invariants asserted. Passes in 0.41s. |
| `agents/code-wiki-agent/src/code_wiki_mcp/server.py` | WikiScanInput.repo_path field + wiki_scan passthrough | VERIFIED | Lines 246-249: repo_path field. Line 272: `Path(...).resolve() if input.repo_path else None` idiom. |
| `agents/code-wiki-agent/tests/unit/test_wiki_scan_input.py` | Schema unit tests for WikiScanInput.repo_path | VERIFIED | 3 tests: default empty, explicit value, regression guard on existing fields. All pass. |
| `agents/code-wiki-agent/tests/integration/test_mcp_e2e.py` | INTEGRATION_GATE-gated 6-tool subprocess E2E test | VERIFIED | 332 lines. Serial subprocess pattern (`_run_server_serial`). All 6 tools in order. INTEGRATION_GATE applied. Skips correctly without gate. |
...
| `test_mcp_cancel.py` | `code_wiki_agent.commands.query.make_llm` | `monkeypatch.setattr` | VERIFIED | Line 86: `"code_wiki_agent.commands.query.make_llm"` — correct importer namespace (CR-01 fix). |
...
| `test_mcp_e2e.py` | `code-wiki-mcp` subprocess | `subprocess.Popen(['uv', 'run', '--package', 'code-wiki-agent', 'code-wiki-mcp'])` | VERIFIED | test_mcp_e2e.py:75-80: Popen call with correct args. |
...
| `test_mcp_e2e.py` | `responses` (JSON-RPC) | Real `code-wiki-mcp` subprocess via stdio | Yes — gated; SUMMARY confirms 6.1s run with all 6 tools returning `isError: False` | FLOWING (gated) |

### Behavioral Spot-Checks

| Behavior | Command | Result | Status |
|----------|---------|--------|--------|
| Cancel test passes (no gate, no cost) | `uv run --package code-wiki-agent pytest agents/code-wiki-agent/tests/integration/test_mcp_cancel.py -x -v` | 1 passed in 0.41s | PASS |
| WikiScanInput schema tests pass | `uv run --package code-wiki-agent pytest agents/code-wiki-agent/tests/unit/test_wiki_scan_input.py -x -v` | 3 passed in 0.51s | PASS |
| E2E test skips without gate | `uv run --package code-wiki-agent pytest agents/code-wiki-agent/tests/integration/test_mcp_e2e.py -x -v` | 1 skipped | PASS |
| Full non-integration suite — no regressions | `uv run --package code-wiki-agent pytest agents/code-wiki-agent/tests/ -x -k "not e2e and not integration"` | 165 passed, 9 deselected in 9.13s | PASS |
...
| DACLI-01 | 08-02 | E2E test launches code-wiki-mcp as stdio subprocess | SATISFIED | test_mcp_e2e.py:75-80 Popen. SUMMARY confirms 6.1s run. |
...
| DACLI-03 | 08-02 | Runs under CODE_WIKI_RUN_INTEGRATION=1 gate | SATISFIED | test_mcp_e2e.py:20-23: INTEGRATION_GATE defined and applied at line 285. |
...
**Test:** Review ROADMAP.md Phase 8 SC#2 and REQUIREMENTS.md MCP-11 ("opt-in gate consistent with v1.0 integration tests") against the fact that test_mcp_cancel.py runs unconditionally with no CODE_WIKI_RUN_INTEGRATION=1 gate.
