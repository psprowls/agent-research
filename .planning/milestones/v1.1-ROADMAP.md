...
**Depends on**: Phase 5 (graph-wiki-agent v1.0 complete)
**Requirements**: PORT-01, PORT-02, PORT-03, PORT-04, PORT-05, PORT-06, EVAL-11, EVAL-12, EVAL-13
**Success Criteria** (what must be TRUE):
  1. Running `graph-wiki-agent query` against the fixture corpus produces citations that follow lattice-wiki iron rules (no hallucinated wikilinks, correct refusal patterns)
  2. Running `graph-wiki-agent ingest` routes source vs work-item pages correctly and generates frontmatter that passes the mechanical lint pass
  3. Running `graph-wiki-agent lint` applies the canonical rule set (not a paraphrased version) — provenance comments in `prompts/` trace every rule to a source path + anchor
...
**Success Criteria** (what must be TRUE):
  1. `GRAPH_WIKI_RUN_EVAL=1 pytest` completes against all 6 agent roles in models.toml on live Bedrock without credential or access errors (BED-01 gate passes in passing)
...
**Success Criteria** (what must be TRUE):
  ...
  3. A single integration test launches `graph-wiki-mcp` as a stdio subprocess, exercises all six tools (`wiki_init`, `wiki_scan`, `wiki_ingest`, `wiki_query`, `wiki_lint`, `wiki_log`) with realistic inputs, and asserts non-error outcomes
  4. The integration test runs under `GRAPH_WIKI_RUN_INTEGRATION=1` (consistent with existing opt-in gate pattern)
**Plans**: TBD
...
**Success Criteria** (what must be TRUE):
  ...
  2. `graph-wiki-agent trace <file>` displays per-subagent cost (input/output tokens × model price) for each fan-out call
  3. `graph-wiki-agent trace <file>` collapses repeated subagent-role groups into a summary line by default; `--expand` drills into the full event stream
**Plans**: 6 plans
Plans:
**Wave 1**
...
**Success Criteria** (what must be TRUE):
  1. Four new shared fragments exist under `agents/graph-wiki-agent/src/graph_wiki_agent/prompts/_fragments/` (`architecture_overview.py`, `style_rules.py`, `log_format.py`, `claude_md_disambiguation.py`), each carrying the standard `# Source: / # Anchor: / # Source-commit:` provenance header
