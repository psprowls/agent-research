...
must_haves:
  truths:
    - "Future-Pat can read a single file (DRIFT-DECISIONS-RAW.md) and see every body diff between vault-io and lattice-wiki-core for all 11 overlapping module rows from spike 002 §A."
  ...
  artifacts:
    - path: "packages/vault-io/DRIFT-DECISIONS-RAW.md"
  ...
  key_links:
    - from: "packages/vault-io/DRIFT-DECISIONS-RAW.md"
...
<objective>
P-A (per CONTEXT.md SQ-01.1): scripted raw diff dump. Produce `packages/vault-io/DRIFT-DECISIONS-RAW.md` containing per-file `diff -u` output between every overlapping vault-io module and its upstream `lattice-wiki-core` counterpart at the pinned SHA. No verdicts, no judgment — just the diffs. P-B reads this file to assign verdicts and land backports.
...
Output: `packages/vault-io/DRIFT-DECISIONS-RAW.md` (per D-03/D-04) and a reusable generator script.
</objective>
...
<task type="auto">
  ...
  <read_first>
    ...
    - packages/vault-io/src/vault_io/ (confirm local file paths)
  ...
  <action>
    ...
    - Pin LOCAL_PKG_REL=packages/vault-io/src/vault_io.
...
<task type="auto">
  ...
  <files>packages/vault-io/DRIFT-DECISIONS-RAW.md</files>
  <read_first>
  ...
  <action>
    Run `bash scripts/drift-diff.sh > packages/vault-io/DRIFT-DECISIONS-RAW.md`. Verify the output file:
    - Has a top-level title (e.g., `# vault-io ⟷ lattice-wiki-core Raw Drift Dump`).
    ...
    - Add a short prose preamble (3–6 lines) immediately under the title noting: source-of-truth role (per DD-03), regeneration command (`bash scripts/drift-diff.sh > packages/vault-io/DRIFT-DECISIONS-RAW.md`), the 11-row + lint-collapsed structure, and a pointer to `packages/vault-io/DRIFT-DECISIONS.md` (forthcoming in plan 02) as the human-verdict companion.
  ...
  <verify>
    <automated>head -30 packages/vault-io/DRIFT-DECISIONS-RAW.md | grep -q '1b45172a9900842b0f8eea525c8270e7fff50605' &amp;&amp; test "$(grep -c '^### ' packages/vault-io/DRIFT-DECISIONS-RAW.md)" -eq 11 &amp;&amp; for m in 'git_state.py' 'append_log.py' 'update_index.py' 'update_tokens.py' 'ingest_work_item.py' 'init_vault.py' 'lint/\*' 'layout_io.py' 'detect_containers.py' 'scan_monorepo.py' 'ingest_source.py'; do grep -qE "^### $m" packages/vault-io/DRIFT-DECISIONS-RAW.md || { echo "MISSING top-level section: $m"; exit 1; }; done &amp;&amp; for f in lint/common.py lint/container.py lint/dependency.py lint/domain.py lint/file_map.py lint/package_sync.py lint/source_sync.py lint/workflow_hints.py; do grep -qF "$f" packages/vault-io/DRIFT-DECISIONS-RAW.md || { echo "MISSING lint sub-file: $f"; exit 1; }; done</automated>
  </verify>
  <acceptance_criteria>
    - `packages/vault-io/DRIFT-DECISIONS-RAW.md` exists.
    - `head -30 packages/vault-io/DRIFT-DECISIONS-RAW.md | grep -q '1b45172a9900842b0f8eea525c8270e7fff50605'` succeeds (SHA pin in header).
    - `grep -c '^### ' packages/vault-io/DRIFT-DECISIONS-RAW.md` equals exactly `11` (one top-level section per overlapping ROW from spike 002 §A).
    ...
    - No source code under `packages/vault-io/src/` was modified by this task (verify with `git status`).
  </acceptance_criteria>
  <done>Raw diff dump committed at `packages/vault-io/DRIFT-DECISIONS-RAW.md` with pinned SHA in header, all 11 row sections present, and all 8 lint sub-file diffs nested under the `### lint/*` row.</done>
</task>
