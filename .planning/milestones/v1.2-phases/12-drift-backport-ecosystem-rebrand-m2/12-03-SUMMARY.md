...
provides:
  ...
  - "packages/wiki-io/src/ rebranded (12 .py + lint/ + assets templates); layout block markers renamed `<!-- graph-wiki:layout:start/end -->`"
  - "agents/graph-wiki-agent/ rebranded (commands prose + architecture_overview prompt fragment)"
...
affects:
  - packages/wiki-io/src/
  - agents/graph-wiki-agent/src/
...
key-files:
  ...
  modified:
    - packages/wiki-io/src/wiki_io/{__init__,init_vault,update_index,scan_monorepo,ingest_work_item,git_state,layout_io,ingest_source,update_tokens,append_log}.py
    - packages/wiki-io/src/wiki_io/lint/{container,dependency}.py
    - packages/wiki-io/src/wiki_io/assets/{AGENTS.md,CLAUDE.md,cursorrules,index.md,log.md}.template
    - packages/wiki-io/src/wiki_io/assets/page-templates/{app,package,source}.md
    - agents/graph-wiki-agent/src/graph_wiki_agent/commands/{scan,lint}.py
    - agents/graph-wiki-agent/src/graph_wiki_agent/prompts/_fragments/architecture_overview.py
    - agents/graph-wiki-agent/tests/prompts/{test_project_context,test_prompt_snapshots}.py
...
decisions:
  ...
  - "Marker rename `<!-- lattice-wiki:layout -->` → `<!-- graph-wiki:layout -->` was bundled with wiki-io src rebrand (commit 1); the rename required updating two agents/graph-wiki-agent test fixtures (test_project_context.py + test_prompt_snapshots.py) in the same commit to keep SQ-03 green-or-revert satisfied."
  - "Test files in packages/wiki-io/tests/ left verbatim: every `lattice` ref is either a provenance docstring or an upstream-guard test (`test_no_lattice_wiki_core_imports` + `assert \"lattice_wiki_core\" not in text`). Renaming the guard literal would break the guard."
...
| --- | --------------------------------------------------------------------------------------------------------- | --------- | ----- | ---------------------------------------------------------------------------------------- |
| 1   | `refactor: rebrand lattice → graph-wiki in packages/`                                                     | `00d3e6f` | 19    | wiki-io src + lint/ + assets templates; layout markers; fixture parity (2 agent tests). |
| 2   | `refactor: rebrand lattice → graph-wiki in agents/graph-wiki-agent`                                        | `4291c94` | 4     | scan.py example path, lint.py work-archive comment, architecture_overview prompt + snapshots. |
| 2b  | `refactor: rebrand lattice → graph-wiki in agents/graph-wiki-agent (followup)`                             | `ccb5876` | 1     | lint.py port-history comment reworded to satisfy strict zero-`lattice_wiki_core` grep.   |
...
2. **Auto-fix Rule 3 inline (Task 1, blocking-issue)** — Renaming `LAYOUT_START` / `LAYOUT_END` constants in `wiki_io/layout_io.py` from `<!-- lattice-wiki:layout:* -->` to `<!-- graph-wiki:layout:* -->` immediately broke 7 fixture-based tests under `agents/graph-wiki-agent/tests/prompts/` that materialize a CLAUDE.md fixture containing the old markers. Per SQ-03 (green-or-revert), the only options were (a) bundle fixture rename with the marker rename in commit 1, or (b) leave markers as `lattice-wiki:layout`. Chose (a) so the rebrand isn't half-done; documented the cross-task file (`agents/graph-wiki-agent/tests/prompts/{test_project_context,test_prompt_snapshots}.py`) in the commit message.

3. **Followup commit for stricter acceptance criterion** — Task 2's first commit left `Import swaps: lattice_wiki_core.* → wiki_io.*` as a port-history docstring in `commands/lint.py`. The acceptance criterion `grep -rE 'lattice_workspace|lattice_wiki_core' agents/graph-wiki-agent/src/` returns zero matches required tightening — reworded to "Imports swap upstream package -> wiki_io for the linter's internals." Created a small followup commit rather than amending.

4. **Asset templates rebranded** — `packages/wiki-io/src/wiki_io/assets/{AGENTS,CLAUDE,cursorrules,index,log}.md.template` and `page-templates/{app,package,source}.md` were not in the plan's `<files>` list but contain `/lattice-wiki:*` slash-command references that get baked into new vaults. Rebranded to keep new-vault prose consistent with the renamed slash-command namespace; documented in commit 1.

5. **`lint/container.py` + `lint/dependency.py` rebranded** — plan's `<files>` list didn't enumerate `lint/`, but the action text says "All `packages/wiki-io/src/wiki_io/*.py` files (12 modules per CONTEXT.md)". Both files had functional lattice references (`/lattice-wiki:init` error message and `lattice-graph` forward-looking ref); rebranded for consistency.
...
| 1    | `grep -rE 'lattice_workspace\|lattice_wiki_core' packages/wiki-io/src/ ...baseline.py ...pricing.py` is empty | ✅ |
| 2    | `uv run pytest` exits 0 after commit 2 (and followup)                                              | ✅      |
| 2    | HEAD subject = `refactor: rebrand lattice → graph-wiki in agents/graph-wiki-agent`                  | ✅      |
| 2    | `grep -rE 'lattice_workspace\|lattice_wiki_core' agents/graph-wiki-agent/src/` is empty             | ✅ (after followup) |
| 3    | `plugins/` exists                                                                                  | ✅      |
