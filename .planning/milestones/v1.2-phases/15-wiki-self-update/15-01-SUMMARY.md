...
affects:
  - ~/Personal/wiki/deep-agents/ (separate git repo — vault-side mutations not coordinated by this commit)
tech_stack_added: []
...
2. **`wiki-config-claude.toml`** — Two-line WikiConfig pointing `models_path` at `models-claude.toml` and `vault_path` at `/Users/pat/Personal/wiki/deep-agents`. `wiki-config.toml` (default Qwen profile) was NOT modified.
...
The wiki vault at `~/Personal/wiki/deep-agents/` is a separate git repo; commits there are out of scope for this plan per D-10. The Phase 15 scan/ingest produced:

- `~/Personal/wiki/deep-agents/packages/workspace-io/workspace-io.md` — **new**, scan-produced. Subject of SC#2 spot-check. Frontmatter parseable; body has 3 key claims about workspace-io's responsibilities (config resolution, manifest IO, init, version tracking).
- `~/Personal/wiki/deep-agents/plugins/graph-wiki/graph-wiki.md` — **new**, scan-produced as a bonus product of the same scan; not required by SC but mentioned as supplementary SC#2 evidence in the verification doc.
- `~/Personal/wiki/deep-agents/sources/otel-story-observability.md` — **new**, ingest-produced. The single OTel source doc summary per D-04. Frontmatter has all required fields; summary cites the source URL and author.
- `~/Personal/wiki/deep-agents/index.md` — refreshed by the scan to include the new `workspace-io` and `graph-wiki` entries.
- `~/Personal/wiki/deep-agents/log.md` — appended with the successful scan-log entry `## [2026-05-18] scan | scan complete: +2 ~0 -0` (SC#1 evidence).
- `~/Personal/wiki/deep-agents/CLAUDE.md` — layout block updated by the executor (auto-fix Rule 1) to replace stale `source: cores` with current `source: packages`, add `plugins` container, remove stale `lattice` container, and bump `children_count`. Documented under Deviation 2 in 15-VERIFICATION.md.
...
1. **[Rule 1 - Bug] `--config` does not propagate `vault_path` to subcommands.** First scan ran against the dogfood vault (`/Users/pat/Personal/deep-agents/graph-wiki/wiki`) instead of `~/Personal/wiki/deep-agents` because `--vault` was not passed. Stale-tagged 28 dogfood pages; fix removed those tags; subsequent invocations always passed `--vault`.
...
- FOUND: `/Users/pat/Personal/wiki/deep-agents/packages/workspace-io/workspace-io.md`
- FOUND: `/Users/pat/Personal/wiki/deep-agents/plugins/graph-wiki/graph-wiki.md`
- FOUND: `/Users/pat/Personal/wiki/deep-agents/sources/otel-story-observability.md`
