...
| `graph_wiki_agent.commands.init.run_init` | `vault_io.init_vault.init_wiki` | `init_wiki(...)` | WIRED | Line 79 receives wiki path from `resolve_wiki_and_repo(vault_path)` at line 75 |
...
| `workspace_io.config.resolve` strict mode | `graph-wiki-agent init` error message | RuntimeError line 74-77 | WIRED | Verified via vault-io tests + workspace-io's `test_resolve_raises_when_no_manifest_found` |
