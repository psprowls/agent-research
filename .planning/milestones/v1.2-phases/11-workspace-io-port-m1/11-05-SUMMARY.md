...
provides:
  ...
  - Zero GRAPH_WIKI_REAL_VAULT_PATH references under agents/graph-wiki-agent/
affects:
...
tech-stack:
  added: []
  patterns:
    - "importlib.metadata.version('graph-wiki-agent') for runtime version introspection (D-13)"
...
key-files:
  ...
  modified:
    ...
    - agents/graph-wiki-agent/src/graph_wiki_mcp/server.py
decisions:
  - "D-07 implemented: graph-wiki-agent init <path> calls workspace_io.init BEFORE resolve_wiki_and_repo"
  - "D-12 implemented: workspace_io.init plugin='graph-wiki-agent' with installed_version == applied_version"
  - "D-13 implemented: version sourced from importlib.metadata.version('graph-wiki-agent') at runtime"
...
# Phase 11 Plan 05: graph-wiki-agent wiring + env-var rebrand sweep — Summary

Wired the workspace-io workspace dependency into `graph-wiki-agent`, made the agent's `init` CLI command perform a two-phase bootstrap (D-07: `workspace_io.init` first, then the existing `resolve_wiki_and_repo` + `init_wiki` flow), and swept all 18 remaining `GRAPH_WIKI_REAL_VAULT_PATH` references under `agents/graph-wiki-agent/` to `GRAPH_WIKI_WORKSPACE` across CLI help strings, MCP tool descriptions, Pydantic Field descriptions, and module docstrings.

## What Was Built

**Task 1 — workspace-io dep + two-phase init wiring**

- `agents/graph-wiki-agent/pyproject.toml`: added `"workspace-io"` to `[project] dependencies` and `workspace-io = { workspace = true }` to `[tool.uv.sources]`.
- `agents/graph-wiki-agent/src/graph_wiki_agent/commands/init.py`:
  - Added `import importlib.metadata` and `from workspace_io import init as _ws_init`.
  - In `run_init()`, computed `repo_root = vault_path.parent if vault_path is not None else Path.cwd()` and invoked `_ws_init(repo_root, plugin="graph-wiki-agent", version=importlib.metadata.version("graph-wiki-agent"))` BEFORE the existing `resolve_wiki_and_repo(vault_path)` call.
  - Rebranded the `vault_path` arg docstring (`GRAPH_WIKI_REAL_VAULT_PATH` → `GRAPH_WIKI_WORKSPACE`).
- `uv sync` resolved the new workspace dep; import smoke test (`from workspace_io import init; from graph_wiki_agent.commands.init import run_init`) passed.

**Task 2 — env-var rebrand sweep**

- `agents/graph-wiki-agent/src/graph_wiki_agent/cli.py`: 6 `typer.Option` `--vault` help strings rebranded (the `init`, `scan`, `ingest source`, `ingest work-item`, `log`, and `lint` commands; lines 433, 455, 476, 514, 539, 573). The `query` command's `--vault` help text was already generic ("Vault path (default: env var)") and is unchanged.
- `agents/graph-wiki-agent/src/graph_wiki_mcp/server.py`: 8 hits rebranded — 1 inline comment (line 105, WikiQueryInput), 2 tool description strings (`wiki_query` line 121, `wiki_ingest` line 328), 5 Pydantic `Field` descriptions (lines 154, 196, 243, 309, 375 spanning WikiLogInput, WikiInitInput, WikiScanInput, WikiIngestInput, WikiLintInput).
- `agents/graph-wiki-agent/src/graph_wiki_agent/commands/log.py`, `commands/query.py`, `config.py`: 3 one-line docstring rebrands.
...
- `commands/init.py`: `grep -c 'from workspace_io import init'` = 1; `grep -c 'importlib.metadata'` = 3 (import + version() call + nothing else, expected ≥1); `grep -c 'plugin="graph-wiki-agent"'` = 1; `grep -c 'GRAPH_WIKI_WORKSPACE'` = 1; `grep -c 'GRAPH_WIKI_REAL_VAULT_PATH'` = 0.
- `uv sync` succeeded; `+ workspace-io==0.1.0` installed editable from `packages/workspace-io`.
- Import smoke test passed: `from workspace_io import init; from graph_wiki_agent.commands.init import run_init` exits 0.
...
- Sweep grep: `grep -rE 'GRAPH_WIKI_REAL_VAULT_PATH' agents/graph-wiki-agent/ --include="*.py"` returns zero lines.
...
- Import smoke test passed: `from graph_wiki_agent import cli; from graph_wiki_mcp import server` exits 0. (Note: the verification command had to write to stderr because the MCP server's `_StdoutGuard` correctly intercepts any non-FastMCP write to stdout — the guard invariant is intact and tested by this very behavior.)
...
| 2    | d8232f1 | chore(11-05): rebrand GRAPH_WIKI_REAL_VAULT_PATH -> GRAPH_WIKI_WORKSPACE in agent |
...
- Modified files verified on disk:
  - FOUND: agents/graph-wiki-agent/pyproject.toml (workspace-io in deps + sources)
  - FOUND: agents/graph-wiki-agent/src/graph_wiki_agent/commands/init.py (importlib.metadata + workspace_io import + _ws_init call at line 68)
  - FOUND: agents/graph-wiki-agent/src/graph_wiki_agent/commands/log.py (GRAPH_WIKI_WORKSPACE)
  - FOUND: agents/graph-wiki-agent/src/graph_wiki_agent/commands/query.py (GRAPH_WIKI_WORKSPACE)
  - FOUND: agents/graph-wiki-agent/src/graph_wiki_agent/config.py (GRAPH_WIKI_WORKSPACE)
  - FOUND: agents/graph-wiki-agent/src/graph_wiki_agent/cli.py (GRAPH_WIKI_WORKSPACE x6)
  - FOUND: agents/graph-wiki-agent/src/graph_wiki_mcp/server.py (GRAPH_WIKI_WORKSPACE x8)
- Commits verified in git log:
  - FOUND: 5080a6a (Task 1)
  - FOUND: d8232f1 (Task 2)
- Final sweep grep `grep -rE 'GRAPH_WIKI_REAL_VAULT_PATH' agents/graph-wiki-agent/` returns zero lines.
