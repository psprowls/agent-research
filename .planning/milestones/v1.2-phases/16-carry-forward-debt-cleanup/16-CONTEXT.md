...
- **D-11 (SWEEP-FU-04: both fixture and live):** (a) Build a synthetic post-rebrand fixture vault for the CI-runnable scanner regression sweep — package pages reflecting current names (`workspace-io`, `prompt-sources`, `vault-io`, etc.), gated test asserting no regression vs. v1.1 baseline. (b) One manual live-vault re-sweep against `~/Personal/graph-wiki/agent-research` (the post-Phase-15 vault), output transcript / scores pasted into `16-VERIFICATION.md`. Mirrors Phase 15's D-08/D-09 pattern: fixture for reproducibility + live run for milestone-grade evidence.
