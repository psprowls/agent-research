...
- [x] **Phase 11: workspace-io Port (M1)** — New `packages/workspace-io/` ported from `lattice-workspace`; wiki-io delegates manifest/path resolution to it (completed 2026-05-18)
...
- [x] **Phase 14: Plugin Port (M3b)** — `lattice-wiki` plugin ported to `plugins/graph-wiki/`; namespace renamed; scripts route through wiki-io (completed 2026-05-19)
- [x] **Phase 15: Wiki Self-Update** — `~/Personal/graph-wiki/agent-research` scanned + ingested against the rebranded ecosystem; naming sweep verified post-port (completed 2026-05-19)
...
**Goal**: A new `packages/workspace-io/` Python package owns workspace bootstrap, manifest IO, and config resolution under the `graph-wiki` brand, and `wiki-io` delegates to it cleanly.
...
**Success Criteria** (what must be TRUE):
  ...
  2. Running any wiki-io command in a directory with a `.graph-wiki.yaml` ancestor resolves the wiki + repo paths without `LATTICE_WORKSPACE` being set; `GRAPH_WIKI_WORKSPACE` is the env override that works.
  3. `wiki-io._workspace.resolve_wiki_and_repo` returns the same explicit-path override behavior as before (MCP boundary contract unchanged), now backed by `workspace_io.config.resolve()`.
  4. The `~/Personal/graph-wiki/agent-researchwiki-config.toml` ↔ `.graph-wiki.yaml` question has a written answer in PROJECT.md Key Decisions (migration script shipped, or decision recorded as "not the same surface").
...
- [x] 11-04-PLAN.md — Rewrite wiki-io._workspace as delegation shim; rename env var in wiki-io src docstrings + 2 tests; add new strict-raises test
- [x] 11-05-PLAN.md — Wire workspace_io.init into graph-wiki-agent init (D-07); rebrand env var across CLI help (6), MCP Field descriptions (8), and 3 docstrings
...
**Goal**: Substantive upstream improvements from `lattice-wiki-core` land in `wiki-io`, deliberate forks stay forked with written rationale, and no `lattice*` symbol survives in the in-scope code/planning surface.
**Depends on**: Phase 11 (wiki-io rebrand assumes `workspace-io` exists and `.graph-wiki.yaml` is the manifest filename)
**Requirements**: BACKPORT-01, BACKPORT-02, BACKPORT-03, BACKPORT-04, BRAND-01, BRAND-02, BRAND-04
**Success Criteria** (what must be TRUE):
  1. `packages/wiki-io/lint/` matches upstream behavior on substantive changes; `packages/wiki-io/DRIFT-DECISIONS.md` records a per-file verdict (port vs. leave) with one-line rationale for each candidate module from spike 002.
...
**Goal**: The open question "what do `lattice-wiki` plugin slash commands actually shell out to?" has a locked answer, and the contract surface between the plugin and agent-research is documented before any plugin code is moved.
**Depends on**: Phase 12 (spec references the rebranded package names and the locked wiki-io/workspace-io API surface)
**Requirements**: PLUGIN-01
**Success Criteria** (what must be TRUE):
  1. A spec doc under `.planning/spec/` enumerates every `lattice-wiki` plugin slash command and names what each one will shell out to (agent-research CLI subcommand, MCP tool, or lattice CLI to be replaced).
  ...
  3. The contract surface is locked in PROJECT.md Key Decisions (or equivalent), so Phase 14 has no open questions left about plugin → agent-research wiring.
**Plans**: 5 plans
- [x] 13-01-PLAN.md — Per-command port specs for /graph-wiki:bootstrap + /graph-wiki:scan (rename verdicts)
...
**Goal**: The `lattice-wiki` Claude Code plugin runs as `plugins/graph-wiki/` against the agent-research vault, with no `lattice_*` imports and at least one slash command exercising the end-to-end path.
**Depends on**: Phase 13 (contract locked); transitively Phase 11 + Phase 12 (wiki-io and workspace-io are the new IO surface)
...
**Success Criteria** (what must be TRUE):
  ...
  3. `grep -r 'lattice_' plugins/graph-wiki/` returns zero hits — plugin scripts call through `wiki-io` (which itself uses `workspace-io`).
  4. `/graph-wiki:query` runs end-to-end from a Claude Code session against `~/Personal/graph-wiki/agent-research` and returns a real librarian answer (manual smoke check).
**Plans**: 3 plans
- [x] 14-01-PLAN.md — Port `wiki_io.lint_wiki` from upstream lattice_wiki_core (VP-01 prereq for plugin lint shim)
- [x] 14-02-PLAN.md — Port `wiki_io.wiki_search` from upstream lattice_wiki_core (VP-01 prereq for plugin query shim)
- [x] 14-03-PLAN.md — Bundled plugin port: manifest `[plugin]` block + plugins/graph-wiki/ scaffold + 6 shims + SC#4 smoke transcript

### Phase 15: Wiki Self-Update
**Goal**: The project's own wiki at `~/Personal/graph-wiki/agent-research` reflects the post-rebrand codebase — new package names, `.graph-wiki.yaml` manifest awareness, plugin port outcomes — so future librarian queries return answers consistent with the shipped code.
**Depends on**: Phase 14 (plugin port complete so the wiki captures the full rebrand surface)
**Requirements**: BRAND-03
**Success Criteria** (what must be TRUE):
  1. `graph-wiki-agent scan ~/Personal/graph-wiki/agent-research` completes against the post-rebrand repo and the resulting `scan-log.md` shows the new package names (`workspace-io`, `graph-wiki` references) without `lattice` artifacts.
  2. `graph-wiki-agent ingest` re-ingests changed package pages so the wiki body reflects the rebrand; spot-check at least one page (e.g., a `workspace-io` package page) exists and is well-formed.
  3. A follow-up `graph-wiki-agent query "what is workspace-io?"` returns a librarian answer that cites the new code paths (not `lattice-workspace`).
**Plans**: 1 plan
...
**Success Criteria** (what must be TRUE):
  ...
  4. `GRAPH_WIKI_RUN_INTEGRATION` opt-in gate semantics are consistent across all gated tests (a single rule documented somewhere central; no test diverges silently).
...
| 14. Plugin Port (M3b) | 3/3 | Complete   | 2026-05-19 |
