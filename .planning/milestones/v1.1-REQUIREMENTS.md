# Requirements Archive: v1.1 Quality Improvements
...
# Requirements: agent-research / graph-wiki-agent — Milestone v1.1 Quality Improvements
...
Close the output-quality gap with `lattice-wiki` by porting its prompt content into `graph-wiki-agent`, then validate the cost-frontier on Bedrock and prove host-level reliability under the DeepAgents CLI.
...
- [ ] **DACLI-01**: End-to-end test launches `graph-wiki-mcp` as a stdio subprocess from a DeepAgents CLI host
...
- [ ] **DACLI-03**: Test runs in CI under an opt-in env-var gate (consistent with `GRAPH_WIKI_RUN_INTEGRATION=1` pattern)

### TRACE — Trace/observability polish

- [ ] **OBS-04**: `.graph-wiki/traces/` JSONL schema is documented and versioned (schema-version field + breaking-change policy)
- [ ] **OBS-05**: `graph-wiki-agent trace` renderer surfaces per-subagent cost (input/output tokens × model price) per trace
