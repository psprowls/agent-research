...
- **After every task commit:** `uv run --package vault-io pytest -x`
- **After every plan wave:** `uv run --package vault-io pytest` (full suite, mock-only)
...
|--------|----------|-----------|-------------------|-------------|
| SCAN-01 | `_load_existing_pages` skips companion files in `wiki/packages/<pkg>/` | unit | `uv run --package vault-io pytest tests/test_scan_companion_fold.py::test_load_existing_skips_companions -x` | ❌ W0 |
| SCAN-01 | Layout-pinned `package` containers also get the companion filter | unit | `uv run --package vault-io pytest tests/test_scan_companion_fold.py::test_layout_pinned_package_skips_companions -x` | ❌ W0 |
| SCAN-01 | `wiki/apps/` is NOT filtered (negative guard) | unit | `uv run --package vault-io pytest tests/test_scan_companion_fold.py::test_apps_not_filtered -x` | ❌ W0 |
...
| WSRES-03 | Synthetic `tmp_path` fixture exercises end-to-end v2 resolution | unit | `uv run --package vault-io pytest tests/test_detect_containers.py::test_v2_synthetic_repo -x` | ❌ W0 |
...
- [ ] `packages/vault-io/tests/test_scan_companion_fold.py` — new file; covers SCAN-01, SCAN-02
- [ ] `packages/vault-io/tests/test_update_tokens.py` — new file (no existing test for `update_tokens.py`); covers TOK-01
- [ ] `packages/vault-io/tests/integration/test_count_tokens_live.py` — new file; gated integration; covers TOK-02 (live Bedrock)
- [ ] `packages/vault-io/tests/test_detect_containers.py` — new file; covers WSRES-01, WSRES-02, WSRES-03
- [ ] Fixture extensions in `packages/vault-io/tests/conftest.py` (or inline) — synthetic v2 monorepo + companion-vault patterns
