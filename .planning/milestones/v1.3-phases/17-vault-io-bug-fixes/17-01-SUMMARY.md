...
requires:
  - phase: 16-carry-forward-debt-cleanup
    provides: "wiki-io lint module including workflow_hints._parse_workflow_hints reused for companion discovery"
provides:
...
tech-stack:
  ...
  patterns:
    ...
    - "Reuse wiki_io.lint.workflow_hints._parse_workflow_hints across modules in same package"
...
## Accomplishments
- Added `from wiki_io.lint.workflow_hints import _parse_workflow_hints` import to `scan_monorepo.py`
...
- `packages/wiki-io/src/wiki_io/scan_monorepo.py` - Added `_parse_workflow_hints` import; companion-fold two-pass logic in `_collect()` and `domains_dir` block
- `packages/wiki-io/tests/test_scan_companion_fold.py` - New: 4 unit tests for companion fold, layout-pinned packages, apps-not-filtered, compute_diff phantom deletes
...
- **Parent overview detection:** `md.stem == md.parent.name` convention (e.g. `wiki-io/wiki-io.md`) identifies parent overviews without reading frontmatter in the first pass.
...
- **Files modified:** `packages/wiki-io/tests/test_scan_companion_fold.py`
...
Files created/modified:
- `packages/wiki-io/src/wiki_io/scan_monorepo.py` — MODIFIED (implementation)
- `packages/wiki-io/tests/test_scan_companion_fold.py` — CREATED (tests)
...
- SCAN-01 and SCAN-02 requirements complete; wiki-io scan diff is no longer polluted by 28 phantom companion deletions
