...
tags: [graph-wiki-agent, cli, mcp, cleanup, deletion, python]
...
provides:
  ...
  - "graph-wiki-agent CLI no longer has a --config global option (Typer @app.callback() removed)"
  - "graph_wiki_mcp.server.main() reduced to mcp.run(transport='stdio') — no GRAPH_WIKI_CONFIG env-var read, no set_models_path import"
...
tech-stack:
  ...
  patterns:
    ...
    - "Docstring scrub: module + dataclass docstrings rewritten verbatim per plan to remove obsolete @app.callback / GRAPH_WIKI_CONFIG usage examples while retaining a single 'removed in Phase 20' breadcrumb for archaeology (intentional residual grep hit — see Deviations §1)"
...
key-decisions:
  ...
  - "No agent-side .md files exist under agents/graph-wiki-agent/ outside the tests directory — Task 3 was a no-op verification, not an edit"
...
# Phase 20 Plan 03: Delete --config / GRAPH_WIKI_CONFIG / models_path Plumbing Summary

**Surgical deletion sweep: `WikiConfig.models_path`, the `--config` Typer option, the `GRAPH_WIKI_CONFIG` env-var read in the MCP server, and the now-dangling `set_models_path` imports left behind by Plan 02 are all removed; test_config.py is brought back into sync with the post-deletion contract.**
...
- **Tests passing:** graph-wiki-agent + full workspace 579/579 (32 integration skips unchanged); model-adapter 21/21; workspace-io invariants hold (within the 579 aggregate)

## Accomplishments

- **Deleted `WikiConfig.models_path` field** from `config.py`. Updated the dataclass docstring to drop the `models_path:` bullet. Rewrote the module docstring verbatim per plan: dropped both code-fence examples (`@app.callback` `--config` AND `os.environ.get("GRAPH_WIKI_CONFIG")`) and replaced with the post-Phase-20 contract description.
...
- **Reduced `graph_wiki_mcp.server.main()` to a two-line body** (comment + `mcp.run(transport="stdio")`). Removed the `import os`, the `GRAPH_WIKI_CONFIG` env-var read, the `graph_wiki_agent.config` import, and the `from model_adapter.loader import set_models_path` import (which would have failed at import time post-Plan-02 if any user ever set `GRAPH_WIKI_CONFIG`). `from pathlib import Path` stayed at module scope — used elsewhere in the file.
...
- **Confirmed Task 3 was a no-op:** `find agents/graph-wiki-agent/ -name '*.md' -not -path '*/tests/*' -not -path '*/.pytest_cache/*'` returned zero files. The recursive `--config` / `GRAPH_WIKI_CONFIG` / `wiki-config.toml` grep gates all return 0 file matches.
- **CLI and MCP server import cleanly** post-deletion. `graph-wiki-agent --help` exits 0 with `--config` absent from the output.

## Task Commits

Atomic commits per the project's recent style:

1. **Task 1:** `382a9cd` — `feat(20-03): delete models_path / --config / GRAPH_WIKI_CONFIG plumbing` — 3 source files, 8 insertions / 44 deletions.
2. **Task 2:** `d72f22a` — `test(20-03): drop models_path / --config assertions from test_config.py` — 1 test file, 4 insertions / 30 deletions.
3. **Task 3:** No commit — no `.md` files under `agents/graph-wiki-agent/` outside tests carry the deleted references. Acceptance gates passed without edits (Karpathy §3 — no edits = no commit).

## Files Created/Modified

- `agents/graph-wiki-agent/src/graph_wiki_agent/config.py` — module docstring rewritten verbatim per plan; `models_path` field deleted from `WikiConfig`; dataclass docstring updated. `load_config`, `get_config`, `_active_config` untouched.
- `agents/graph-wiki-agent/src/graph_wiki_agent/cli.py` — `@app.callback()` + `main_callback` deleted (10 lines removed); `set_models_path` import removed (it was inside the deleted callback body). All subcommands and the `Optional` import preserved.
- `agents/graph-wiki-agent/src/graph_wiki_mcp/server.py` — `main()` body reduced from 13 lines to 2 (preserved the RESEARCH A2 transport comment). `import os` (function-scoped) dropped. `Path` import at module scope kept — used elsewhere in the file.
- `agents/graph-wiki-agent/tests/unit/test_config.py` — module docstring rewritten; one test renamed (`_parses_remaining_fields`), one test deleted (`test_typer_callback_sets_active_config`); `models_path` lines stripped from the two remaining `load_config` tests. Test count: 4 → 3.

## Files Touched in Task 3 Docs Sweep

**None.** The enumeration command `grep -rln --include='*.md' --include='README*' --exclude-dir='tests' --exclude-dir='.pytest_cache' -- '--config\|GRAPH_WIKI_CONFIG' agents/graph-wiki-agent/ 2>/dev/null` returned no matches; `find agents/graph-wiki-agent/ -name '*.md' -not -path '*/tests/*' -not -path '*/.pytest_cache/*'` returned no files. The agent has no live `.md` docs to sweep.

## Decisions Made

- **Verbatim docstring text takes precedence over the literal grep gate.** Plan Task 1 step 1a says "Do not paraphrase — use this exact string." The mandated docstring contains the breadcrumb `the \`--config\` / GRAPH_WIKI_CONFIG pathway was removed in Phase 20 / WMC-03` for archaeological context. This creates one residual match per grep gate (`grep -c '@app.callback\|--config\|GRAPH_WIKI_CONFIG\|set_models_path'` returns 1, not 0; gate #2 of the phase verification also returns 1). The plan's verbatim text spec wins; the grep gate is over-restrictive against the very docstring it mandates. See Deviations §1.
...
- **Found during:** Task 2 verification (gate `grep -c -- '--config' agents/graph-wiki-agent/tests/unit/test_config.py == 0`).
- **Issue:** Plan Task 2 step 1 mandates rewriting the module docstring to: `"Unit tests for graph_wiki_agent.config module — exercises load_config TOML parsing and the _active_config singleton. (CLI-05 / --config plumbing was removed in Phase 20 / WMC-03.)"`. That mandated text contains `--config`; gate returns 1, not 0.
...
- **Issue:** `find agents/graph-wiki-agent/ -name '*.md' -not -path '*/tests/*' -not -path '*/.pytest_cache/*'` returned no files. The recursive grep against `--config` / `GRAPH_WIKI_CONFIG` / `wiki-config.toml` in `.md` / `README*` (with the tests + pytest_cache exclusions) returned 0 matches without any edit. The acceptance gates passed natively.
...
None — pure source/test deletion sweep inside `agents/graph-wiki-agent/`. The two formerly-supported user surfaces (`graph-wiki-agent --config <toml>` and `GRAPH_WIKI_CONFIG=<toml>`) are deleted; users who relied on them must switch to `<workspace>/.graph-wiki.yaml` `plugins[].roles[]` (delivered by Plans 20-01 and 20-02).
...
- `agents/graph-wiki-agent/src/graph_wiki_agent/config.py` — VERIFIED MODIFIED (commit `382a9cd`)
- `agents/graph-wiki-agent/src/graph_wiki_agent/cli.py` — VERIFIED MODIFIED (commit `382a9cd`)
- `agents/graph-wiki-agent/src/graph_wiki_mcp/server.py` — VERIFIED MODIFIED (commit `382a9cd`)
- `agents/graph-wiki-agent/tests/unit/test_config.py` — VERIFIED MODIFIED (commit `d72f22a`)
...
Task 1 gates:
- `grep -c 'models_path' agents/graph-wiki-agent/src/graph_wiki_agent/config.py` → 0 — PASS
- `grep -c '@app.callback\|--config\|GRAPH_WIKI_CONFIG\|set_models_path' agents/graph-wiki-agent/src/graph_wiki_agent/config.py` → 1 — DEVIATION §1 (plan-mandated docstring text)
- `grep -c 'models_path\|--config\|set_models_path\|main_callback' agents/graph-wiki-agent/src/graph_wiki_agent/cli.py` → 0 — PASS
- `grep -c 'GRAPH_WIKI_CONFIG\|set_models_path' agents/graph-wiki-agent/src/graph_wiki_mcp/server.py` → 0 — PASS
- `uv run --package graph-wiki-agent python -c "from graph_wiki_agent.cli import app; from graph_wiki_mcp import server"` exit 0 — PASS
- `graph-wiki-agent --help` exit 0 with `--config` absent — PASS (0 matches)
- `python -c "from graph_wiki_agent.config import WikiConfig, load_config, get_config"` exit 0 — PASS

Task 2 gates:
- `grep -c 'models_path' agents/graph-wiki-agent/tests/unit/test_config.py` → 0 — PASS
- `grep -c 'test_typer_callback_sets_active_config' agents/graph-wiki-agent/tests/unit/test_config.py` → 0 — PASS
- `grep -c -- '--config' agents/graph-wiki-agent/tests/unit/test_config.py` → 1 — DEVIATION §2 (plan-mandated docstring text)
- `grep -c '^def test_' agents/graph-wiki-agent/tests/unit/test_config.py` → 3 — PASS
- `uv run --package graph-wiki-agent pytest tests/unit/test_config.py -x` → 3/3 passed — PASS
- `uv run --package graph-wiki-agent pytest -x` → 579/579 passed (32 integration skips) — PASS

Task 3 gates:
- `grep -rln --include='*.md' --include='README*' --exclude-dir='tests' --exclude-dir='.pytest_cache' -- '--config\|GRAPH_WIKI_CONFIG' agents/graph-wiki-agent/ | wc -l` → 0 — PASS
- `grep -rln --include='*.md' --exclude-dir='tests' --exclude-dir='.pytest_cache' 'wiki-config\.toml' agents/graph-wiki-agent/ | wc -l` → 0 — PASS
- `uv run --package graph-wiki-agent pytest -x` → 579/579 passed — PASS

Phase verification gates:
- `uv run --package graph-wiki-agent pytest -x` → 579 passed, 32 skipped — PASS
...
- `grep -rn "set_models_path\|GRAPH_WIKI_CONFIG\|models_path" agents/graph-wiki-agent/src/` → 1 hit (docstring breadcrumb on `config.py:12`, DEVIATION §1) — INTENDED RESIDUAL
- `graph-wiki-agent --help` exit 0; `--config` absent — PASS
- `python -c "from graph_wiki_mcp import server"` exit 0 — PASS
