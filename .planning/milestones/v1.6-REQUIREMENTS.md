...
**Status:** ✅ ARCHIVED 2026-05-26 — all 56 requirements satisfied (30 checkboxes flipped during execution; 26 verified-but-unflipped — STRUCT-01..06, SPARSER-01..02, CLI-01..14, BRAND-01..04 — flipped at close since the phases shipped with passing SC checks).
...
- [x] **STRUCT-01**: Scanner emits one `Repository` node per scanned repo, carrying URL/default-branch/owner attrs derived from `git remote` and `git config` when available
- [x] **STRUCT-02**: Scanner emits `SubPackage` nodes (Python-only) for each `__init__.py`-containing subdirectory below a package root; JS/TS scans do NOT emit `SubPackage` nodes (directory paths live as attributes on `File` per spec §3)
- [x] **STRUCT-03**: `File` nodes carry role-flag attrs `is_importable`, `is_executable`, `has_main`, `is_test`, `is_config`, `is_generated`, `is_type_only` — flags are independent booleans (a file can be both `is_importable` and `is_executable`)
- [x] **STRUCT-04**: `physically_contains` edges form a strict tree — each node has exactly ONE structural parent (Repository → Package → [Python: SubPackage* → File | JS/TS: File]); test files are NOT under Package containment (they live under TestSuite)
- [x] **STRUCT-05**: Generic container folders (`packages/`, `libs/`, `tests/`, `apps/`, `shared/`, `common/`) are NEVER emitted as nodes — their paths exist only as attributes on the nodes they contain
- [x] **STRUCT-06**: `resolve.sweep` cleanup excludes the new node kinds (`repository`, `domain`, `test_suite`, `entry_point`) from `path=NULL` deletion — prevents silent loss of structural/conceptual nodes that have no source-file path
...
- [x] **SPARSER-01**: `source-parser` Python parser populates `_has_main_block` (detects `if __name__ == "__main__":`) and `_has_importable_symbols` (detects presence of public top-level definitions) on file-level `SourceNode.attrs`
- [x] **SPARSER-02**: `graph-io` reads these source-parser attrs to refine `File.has_main` and `File.is_importable` role flags; other role flags (`is_test`, `is_config`, `is_generated`, `is_type_only`) remain path-heuristic-only in graph-io
...
- [x] **CLI-01**: `cg describe-repo` describes the current repository (URI, default branch, owner, packages contained)
- [x] **CLI-02**: `cg list-packages` lists all `Package` nodes in the DB
- [x] **CLI-03**: `cg list-entry-points <pkg> [--kind executable|library]` lists entry points declared by a package
- [x] **CLI-04**: `cg list-scripts` returns the union of declared EntryPoints with `kind:executable` and Files with `is_executable: true` (spec §10 query "What scripts exist in this repo?")
- [x] **CLI-05**: `cg list-suites` lists all `TestSuite` nodes
- [x] **CLI-06**: `cg describe-suite <name>` describes a TestSuite (kind, framework, files contained, packages tested)
- [x] **CLI-07**: `cg what-tests <package>` returns TestSuite nodes that test a package (via suite-level `tests` edges)
- [x] **CLI-08**: `cg what-tests <domain>` returns TestSuite nodes that test a domain (via `TestSuite → Domain` edges OR via `TestSuite → Package` where the package `belongs_to_domain`) — spec §10 "What integration tests touch the Billing domain?"
- [x] **CLI-09**: `cg list-domains` lists all `Domain` nodes
- [x] **CLI-10**: `cg describe-domain <name>` describes a domain (packages, sub-domains, references, depends-on)
- [x] **CLI-11**: `cg domain-refs <name>` shows packages referenced by a domain with usage counts (spec §10 "What does the Billing domain depend on outside of itself?")
- [x] **CLI-12**: `cg domain-deps <name>` shows domains that a domain depends on
- [x] **CLI-13**: `cg cross-cutting` shows packages with zero `belongs_to_domain` edges, ranked by incoming `references` count (spec §10 "Which utility packages are most widely used?")
- [x] **CLI-14**: `cg status` extended to surface the repository URI alongside the existing staleness check

### Brand Sweep (graph-io only)

- [x] **BRAND-01**: `packages/graph-io/README.md` rebranded — `lattice-graph-core` → graph-wiki phrasing; `~/.lattice/graph/code.db` → canonical graph-wiki path (resolved via `workspace_io.paths.graph_dir`)
- [x] **BRAND-02**: All `cg` CLI description/help strings use graph-wiki branding; no surviving `lattice` references in any user-facing string emitted by graph-io
- [x] **BRAND-03**: `LATTICE_GRAPH_LOCK_TIMEOUT_MS` env var renamed to `GRAPH_WIKI_LOCK_TIMEOUT_MS` (or workspace-consistent name); deprecation alias preserves backward compat for one milestone with a deprecation warning
- [x] **BRAND-04**: Sweep scope is LIMITED to `packages/graph-io/` and its consumers — does NOT touch `plugins/graph-wiki/`, `packages/wiki-io/`, or any other package; explicitly NOT touching `_SKIP_REPO_PREFIXES = ("lattice/",)` in `packages.py` (functional behavior, not brand text, per PITFALLS.md)
...
| STRUCT-01 | Phase 29 | Complete |
| STRUCT-02 | Phase 29 | Complete |
| STRUCT-03 | Phase 29 | Complete |
| STRUCT-04 | Phase 29 | Complete |
| STRUCT-05 | Phase 29 | Complete |
| STRUCT-06 | Phase 29 | Complete |
| SPARSER-01 | Phase 29 | Complete |
| SPARSER-02 | Phase 29 | Complete |
...
| CLI-07 | Phase 33 | Complete |
| CLI-08 | Phase 33 | Complete |
| CLI-09 | Phase 33 | Complete |
| CLI-10 | Phase 33 | Complete |
| CLI-11 | Phase 33 | Complete |
| CLI-12 | Phase 33 | Complete |
| CLI-13 | Phase 33 | Complete |
| CLI-14 | Phase 33 | Complete |
| BRAND-01 | Phase 34 | Complete |
| BRAND-02 | Phase 34 | Complete |
| BRAND-03 | Phase 34 | Complete |
| BRAND-04 | Phase 34 | Complete |
