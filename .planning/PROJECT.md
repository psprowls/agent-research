...
| **Plugin shell-out via `uv run --project "$DEEP_AGENTS_ROOT"`** (SO-01, v1.2) | Plugin scripts at `plugins/graph-wiki/scripts/*.py` shell out to agent-research Python helpers via `uv run --project "$DEEP_AGENTS_ROOT" python3 -m ...`; backend selection per command via `[plugin]` block in `.graph-wiki.yaml` (SO-03), defaulting to `claude` everywhere with `bedrock` as documented per-command opt-in. | ✓ Validated Phase 14 |
