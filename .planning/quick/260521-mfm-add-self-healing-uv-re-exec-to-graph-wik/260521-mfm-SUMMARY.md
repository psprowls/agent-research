...
Make `python <plugin-shim>.py` work from any shell, not just under `uv run --project packages/vault-io`. Previously, bare-`python` invocations failed with `ModuleNotFoundError: No module named 'vault_io'` because the shims import workspace packages at top level.

## What changed

- **New file** `plugins/graph-wiki/skills/graph-wiki/scripts/_uv_reexec.py` — 45-line bootstrap helper. `ensure()` tries `import vault_io`; on `ImportError`, walks up from `__file__` to find `packages/vault-io/pyproject.toml`, then `os.execvpe`s under `uv run --project <pkg> python <self> <args...>`. A `GRAPH_WIKI_SHIM_REEXEC=1` env-var guard prevents infinite loops if uv itself can't satisfy the import.
- **Wired into 6 shims** — `detect_containers.py`, `ingest_source.py`, `init_vault.py`, `lint_wiki.py`, `scan_monorepo.py`, `wiki_search.py`. Each calls `_ensure_uv()` immediately before the existing `from vault_io...` import.
...
Re-exec via uv succeeds; help text from the real `vault_io.detect_containers` is printed.
...
Traceback (most recent call last):
  …
ModuleNotFoundError: No module named 'vault_io'
exit: 1
```
