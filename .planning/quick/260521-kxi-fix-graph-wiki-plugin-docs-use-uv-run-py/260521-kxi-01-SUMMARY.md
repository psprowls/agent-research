...
requires: []
provides:
  - "All graph-wiki plugin docs invoke bundled shims via `uv run --project \"$AGENT_RESEARCH_ROOT\" python …` so `wiki_io` resolves"
  - "SKILL.md Python-tools preamble accurately describes the shim model (in-workspace `wiki_io` import, not stdlib-only)"
affects:
...
key_decisions:
  ...
  - "Replace stale `Standard library only (via wiki_io)` claim in SKILL.md with prose that accurately describes the shim-imports-`wiki_io` model"
...
The shims under `plugins/graph-wiki/skills/graph-wiki/scripts/` are thin `from wiki_io.<tool> import main` wrappers; `wiki_io` is a `uv` workspace member (`packages/wiki-io/`) and is only importable inside the uv-managed venv. Bare `python …shims…` therefore fails with `ModuleNotFoundError: No module named 'wiki_io'`. The fix is to invoke through the workspace.
...
| 2 | Correct stale `Standard library only (via wiki_io)` claim in SKILL.md Python-tools section | 36f4d56 | plugins/graph-wiki/skills/graph-wiki/SKILL.md |
...
3. **End-to-end invocation works.** Ran `uv run --project "$AGENT_RESEARCH_ROOT" python ${CLAUDE_PLUGIN_ROOT}/skills/graph-wiki/scripts/detect_containers.py --help` from `/tmp` with `AGENT_RESEARCH_ROOT` and `CLAUDE_PLUGIN_ROOT` set. Printed argparse `--help` text from `wiki_io.detect_containers` — no `ModuleNotFoundError`.
