...
Last activity: 2026-05-30 — Completed quick task 260530-hxy: Fix graph build repo resolution to match scan (`graph build` now resolves the source repo from cwd like `scan`, fixing the repo≠workspace `fatal: ambiguous argument 'HEAD'`; `repo-directory:` pin now honored by both). Phase 60 (cost-frontier sweep) deliberately parked until the graph is trustworthy + a clean baseline can be established — addressing graph/bootstrap quality todos first.
