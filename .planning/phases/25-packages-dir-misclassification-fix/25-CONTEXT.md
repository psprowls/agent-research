...
Fix the `_classify_dir` heuristic in `packages/vault-io/src/vault_io/detect_containers.py` so a top-level directory containing a mix of manifested package children and one or more non-manifested siblings (or loose `.md` files) is classified as `package` — not `ambiguous`. Update the plugin-side reference doc that describes the heuristic. Add unit tests pinning the new behavior. Resolve the pending todo.
...
### Plugin shim
- **D-06:** `plugins/graph-wiki/skills/graph-wiki/scripts/detect_containers.py` is already a 9-line passthrough (`from vault_io.detect_containers import main`). Success criterion 3 ("plugin-side classifier matches the updated heuristic") is **auto-satisfied** by the vault-io change. No separate plugin port.
...
### Test coverage (`packages/vault-io/tests/test_detect_containers.py`)
...
### Code (primary)
- `packages/vault-io/src/vault_io/detect_containers.py` — the file Phase 25 edits; classifier logic in `_classify_dir` (lines 80-145)
- `packages/vault-io/src/vault_io/init_vault.py` §`_resolve_pinned_containers` (lines 84-133) — consumer of classifier output; `if cls == "ambiguous"` branch stays
- `packages/vault-io/tests/test_detect_containers.py` — where new unit tests land
- `plugins/graph-wiki/skills/graph-wiki/scripts/detect_containers.py` — thin shim; **read but do not edit** (auto-inherits the vault-io change per D-06)
...
### Reusable Assets
- `packages/vault-io/tests/helpers.py` — provides `tmp_repo`, `write_pkg`, `write_file`, `write_claude_plugin` for inline throwaway repos. The new unit tests should use these helpers — do NOT reach into the repo-root `fixtures/` directory (those are for layout-level tests).
...
- Plugin-side scripts are thin shims importing from `vault_io.<name>` — this phase preserves that pattern; no behavior in the plugin shim itself changes.
...
- The bug was reproduced on this repo on 2026-05-20. `packages/` has 6 children — 5 with `pyproject.toml` (`core-bedrock`, `eval-harness`, `model-adapter`, `subagent-runtime`, `vault-io`, `workspace-io`) and 1 without (`prompt_sources/`). The new rule must classify this case as `package`, with `prompt_sources/` silently excluded.
...
- Verification: after the fix, running `python -m vault_io.detect_containers --json` on this repo should emit `"packages"` with `"classification": "package"` and `"children_count": 5`.
