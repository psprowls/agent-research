...
| **Config file** | `packages/wiki-io/pyproject.toml` (workspace member) |
...
- **After every task commit:** Run `uv run --package wiki-io pytest tests/test_detect_containers.py -x`
...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 25-01-01 | 01 | 1 | PKGCLS-01 | — | N/A | unit | `uv run --package wiki-io pytest tests/test_detect_containers.py::test_mixed_manifest_dirs_classify_as_package -x` | ❌ W0 | ⬜ pending |
| 25-01-02 | 01 | 1 | PKGCLS-01 | — | N/A | unit | `uv run --package wiki-io pytest tests/test_detect_containers.py::test_loose_md_file_at_container_root_does_not_block_package -x` | ❌ W0 | ⬜ pending |
| 25-01-03 | 01 | 1 | PKGCLS-01 | — | N/A | unit | `uv run --package wiki-io pytest tests/test_detect_containers.py::test_empty_dir_falls_back_to_ambiguous -x` | ❌ W0 | ⬜ pending |
| 25-01-04 | 01 | 1 | PKGCLS-01 | — | N/A | unit | `uv run --package wiki-io pytest tests/test_detect_containers.py -x` | ✅ existing | ⬜ pending |
| 25-01-05 | 01 | 1 | PKGCLS-02 | — | N/A | smoke | `python -c "from wiki_io.detect_containers import main"` | ✅ existing | ⬜ pending |
...
- [ ] `packages/wiki-io/tests/test_detect_containers.py` — 3 new unit tests (`test_mixed_manifest_dirs_classify_as_package`, `test_loose_md_file_at_container_root_does_not_block_package`, `test_empty_dir_falls_back_to_ambiguous`); existing `conftest.py` already provides `tmp_repo`/`write_file` helpers — no new fixtures needed.
