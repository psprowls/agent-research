...
provides:
  - --expand boolean flag on `code-wiki-agent trace` (Typer Option)
...
- `code-wiki-agent trace --help` now lists `--expand` (verified by `test_trace_command_has_expand_flag`).
...
- Renderer still has zero `eval_harness` imports — verified by `grep -E "from eval_harness|import eval_harness" agents/code-wiki-agent/src/code_wiki_agent/cli.py` returning 0 (D-10 invariant).
...
- **`agents/code-wiki-agent/tests/unit/__snapshots__/test_trace_viewer.ambr`** (created)
...
None — purely additive renderer extension. No env vars, no migrations, no schema changes. Existing `code-wiki-agent trace <file>` invocations get the new collapsed output by default; users who want the pre-Phase-9 behavior pass `--expand`.
...
- **ROADMAP success criterion 3** ("`code-wiki-agent trace <file>` collapses repeated subagent-role groups into a summary line by default; `--expand` drills into the full event stream") is satisfied:
...
- `agents/code-wiki-agent/src/code_wiki_agent/cli.py` — FOUND (modified, staged + committed in `24e68a4`)
- `agents/code-wiki-agent/tests/unit/test_trace_viewer.py` — FOUND (modified, staged + committed in `e45ac6d`, `24e68a4`, `077f3e9`)
- `agents/code-wiki-agent/tests/unit/__snapshots__/test_trace_viewer.ambr` — FOUND (created in `24e68a4`, snapshots recorded in `066321f`)
...
- Task 1: `code-wiki-agent trace --help` lists `--expand` — PASS
- Task 1: `--expand` substring present in `cli.py` — PASS
- Task 1: zero `eval_harness` imports in `cli.py` — PASS
- Task 1: `pytest agents/code-wiki-agent/tests/unit/test_trace_viewer.py -x` — PASS (11 → 16 passed, 0 failed)
