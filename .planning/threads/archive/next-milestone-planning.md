...
> **Resolved 2026-05-19.** All captured work shipped as milestone v1.2:
> M1 → Phase 11, M2 → Phase 12, M3a/b → Phases 13–14, wiki self-update → Phase 15, cross-cutting cleanup → Phase 16. See `.planning/milestones/v1.2-ROADMAP.md`.
...
| 2026-05-17 | `wiki-io` keeps its name. No rename. | user reply |
| 2026-05-17 | Bring `lattice-workspace` over as a **new** `workspace-io` package (sibling to `wiki-io`). `wiki-io` will depend on `workspace-io` for wiki bootstrap, mirroring how `lattice-wiki-core` depends on `lattice-workspace`. | user reply |
...
- `packages/wiki-io/` — stays. Depends on `workspace-io` for path/manifest/init concerns
- `agents/code-wiki-agent/` — unchanged in structure; may need minor updates to consume `workspace-io` via wiki-io
...
- Update `wiki-io/_workspace.py::resolve_wiki_and_repo` so it **delegates to** `workspace_io.config.resolve()` instead of the env-var-only stub. Keep the explicit-path argument override path (don't regress the MCP boundary contract).
...
- Does the workspace bootstrap (`init.py`) also create the wiki tree, or only the workspace shell? If only shell, `wiki-io.init_vault.init_wiki` still runs after to populate `wiki/`.

### M2 — Selective drift backport from `lattice-wiki-core` into `wiki-io` + `graph-wiki` rebrand
...
| `append_log.py` | +30 | wiki-io is ahead (MCP lib-ification) | **leave**; upstream is older |
| `update_index.py` | +29 | wiki-io is ahead (lib API) | **leave** |
| `update_tokens.py` | +6 | wiki-io is ahead (no tiktoken) | **leave** |
| `ingest_work_item.py` | -1 | API divergence | Decide: wiki-io's `file_work_item` lib shape vs lattice's `_run_helper` dispatcher. **Recommend leave** — wiki-io's shape fits MCP. |
...
| `layout_io.py` | -98 | wiki-io stripped `ensure_package_pages` | **leave** (we skipped package-family — user decision above) |
| `detect_containers.py` | -129 | wiki-io stripped package-family helpers | **leave** (same reason) |
| `scan_monorepo.py` | -151 | wiki-io stripped package-family helpers | **leave** (same reason) |
| `ingest_source.py` | -181 | wiki-io is library-only (CLI moved to commands/) | **leave** — architectural choice |

**The ecosystem rebrand happens here too:**
- Grep `lattice` / `LATTICE` / `lattice_wiki_core` everywhere in `packages/wiki-io/`, `agents/code-wiki-agent/`, `.planning/`, `CLAUDE.md`.
...
**Dependency:** after M1 (wiki-io will use workspace-io's path/manifest API).

### M3 — Bring `lattice-wiki` plugin into `plugins/graph-wiki/` (deferred to own scoping)

**Status:** scope this in a follow-up `/gsd-spike` or `/gsd-spec-phase`. Pre-spike-002 capture below remains valid in shape; the rename targets are now `graph-wiki` (plugin id), `/graph-wiki:*` (command namespace), and plugin scripts must consume `wiki-io` (which itself uses `workspace-io`).
...
- `packages/wiki-io/` → renamed to `packages/workspace-io/`
- `agents/code-wiki-agent/` — existing target for wiki-core logic
- `plugins/` — currently empty; will host `plugins/graph-wiki/`

### T1 — Rename `wiki-io` → `workspace-io` (rebrand step 1)

**Scope:** mechanical rename. Directory move, `pyproject.toml` `name`, all `from wiki_io` imports, workspace member entry in root `pyproject.toml`, any references in `code-wiki-agent` and in `.planning/`.
...
- Resolve overlapping functionality: `wiki-io` already has its own implementation. Define the merge strategy per module (replace, merge field-by-field, or wrap).
...
- **Naming consistency sweep:** at the end of T1–T4, grep for `lattice`, `wiki-io`, `wiki_io` across `packages/`, `agents/`, `plugins/`, `.planning/`, `CLAUDE.md` — anything remaining is a bug.
