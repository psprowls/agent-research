...
**Scan-specific note:** The Claude branch of `scan_monorepo.py` defaults to a commit-gated **emit → fan-out → apply** pipeline (NOT structural-only by default). `--emit-worklist <path>` drives the emit phase (writes entity pages, serializes `worklist.json`, renders `briefs/<page-stem>.md` per stale entity, resets `results/`). The apply phase takes `--results-dir <dir>` (per-entity result files written by the fan-out), `--apply-worklist <results.json>` (a combined document), or both — they merge in one call, with the directory winning uri-wise — plus `--short-head <sha>`. A bare invocation is the `--no-narrate` structural-only fast path (no worklist, no briefs, no prose generated).
...
The wiki lives at `<workspace>/wiki/`. `workspace_io` resolves the workspace from `GRAPH_WIKI_WORKSPACE` — the only external-workspace pointer, injected via the `env` block of the `.claude/settings.local.json` belonging to whichever directory the session runs from (the repo-side `.graph-wiki.local.yaml` `workspace-directory` key is dead). Without it, resolution falls back to `<repo>/graph-wiki/` via a `.git` walk-up; it never searches for a `.graph-wiki.yaml`, so a workspace kept elsewhere is unreachable without the env var or an explicit `--workspace`. The Obsidian vault opens at the workspace root, so `<workspace>/raw/` and `<workspace>/knowledge/` (managed by `workspace_io` and other plugins) are siblings of `<workspace>/wiki/`, not subdirectories of it. `<workspace>/work/` is the exception: it lives at `<workspace>/wiki/work/`, nested under `wiki/`, so `[[work/foo]]` wikilinks resolve the same way as other wiki pages.
