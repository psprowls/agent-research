...
| `modelTier` | "mechanical" \| "standard" \| "frontier" | no | Abstract capability tier for subagents executing this task. Belongs to the opt-in model-routing flow: when `workflow.model_routing` has a non-empty tier mapping (set via `gw config set workflow.model_routing.<tier> <model>`, projected to `<workspace>/.graph-wiki/config.json`), plugin gates require it on every plan task (`pre-taskcreate-model-tier`) and enforce the resolved Agent `model` at dispatch (`pre-agent-model-routing`); the session-start notice carries the rules. Mapping absent/empty → gates are dormant, the key is inert metadata, and every subagent inherits the session model. `"inherit"` as a mapping value means omit the Agent `model` param. A concrete `model` pin (above) always wins over the tier. Full design: `docs/model-routing-flow.md`. |
