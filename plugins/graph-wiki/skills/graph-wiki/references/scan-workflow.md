...
Run `uv run --project "$AGENT_RESEARCH_ROOT" python ${CLAUDE_PLUGIN_ROOT}/skills/graph-wiki/scripts/scan_monorepo.py --json` to get a structured inventory (repo discovered automatically via `workspace_io`):
...
Run `uv run --project "$AGENT_RESEARCH_ROOT" python ${CLAUDE_PLUGIN_ROOT}/skills/graph-wiki/scripts/update_index.py` or edit `<workspace>/wiki/index.md` inline.
