...
- **`<workspace>/raw/<...>`** — clipped articles, specs, PRs, transcripts you've staged. File contents are never edited; after a successful ingest the source is moved to `raw/_archive/<same relative path>`, so `raw/` (outside `_archive/`) only holds un-ingested material. Owned by `workspace_io`.
...
- Link from the source page and from touched concept pages
