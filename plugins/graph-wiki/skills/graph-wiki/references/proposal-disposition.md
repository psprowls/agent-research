...
| Action | Command (set `export GRAPH_WIKI_WORKSPACE=<workspace>` first) |
|---|---|
| List open proposals | `uv run --package graph-wiki-cli gw wiki proposals` |
...
Omit the `tokens:` key (stamped later by `gw wiki tokens`). Write real prose grounded in
...
(Runs in this `uv` workspace; an installed/`uv tool` user without the workspace checked out would instead re-run an ingest/scan to refresh indexes.) Optionally run `gw wiki tokens` afterward to stamp the `tokens:` keys you omitted.

### 5. Verify and archive

```bash
uv run --package graph-wiki-cli gw wiki lint            # 0 broken links / orphans / index drift
uv run --package graph-wiki-cli gw wiki archive --dry-run   # preview: sweeps created/approved/rejected notes
uv run --package graph-wiki-cli gw wiki archive             # moves spent notes to proposals/_archive/
```

Optionally append a `gw wiki log` entry to mirror ingest. Leave commit/push to the user.
