...
**Important:** point Obsidian at `<workspace>/` (the graph-wiki workspace root), not at the wiki subdir. Obsidian indexes whatever folder you open as a vault, and the workspace root is the level where `wiki/`, `raw/`, `work/`, and `knowledge/` are all visible as sibling folders — they're all part of the same compounding knowledge base.

1. Obsidian → "Open folder as vault" → pick `<workspace>/`, e.g. `<repo>/graph-wiki/`. The Obsidian sidebar will show `wiki/`, `raw/`, `work/`, etc. as siblings. Naming the workspace dir something distinctive (the default `graph-wiki/` is fine) makes it obvious which repo the vault belongs to when several are open.
2. The repo root sits one level up — your repo-level `CLAUDE.md`, source code, and build artifacts stay outside Obsidian's view.

**Note on CLAUDE.md files:** there are typically three relevant files when working from a graph-wiki workspace: the repo's root `CLAUDE.md` (build/style conventions), `<workspace>/CLAUDE.md` (workspace-level schema, owned by `workspace_io`), and `<workspace>/wiki/CLAUDE.md` (wiki schema, owned by this plugin). Claude Code loads all three by walking the tree; they describe different layers and don't conflict.
...
- **Use `[[Wikilinks]]`:** ON
- **Attachment folder path:** `raw/assets/` (clipped images go to the workspace's `raw/` layer alongside the wiki, owned by `workspace_io`)
