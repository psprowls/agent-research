...
plugin tree but depend on `vault_io`, a workspace package under
`packages/vault-io/`. When a user invokes a shim with bare `python <shim>`
(outside `uv run`), the `from vault_io...` import fails. This helper detects
that case and re-execs the current script under
`uv run --project <repo>/packages/vault-io python <self> <args...>` so the
...
def ensure() -> None:
    ...
    # If vault_io is already importable, we are inside the uv workspace; bail out.
    try:
        import vault_io  # noqa: F401
        return
    except ImportError:
        pass

    # Walk up from this file looking for packages/vault-io/pyproject.toml.
    here = Path(__file__).resolve().parent
    while True:
        candidate = here / "packages" / "vault-io" / "pyproject.toml"
