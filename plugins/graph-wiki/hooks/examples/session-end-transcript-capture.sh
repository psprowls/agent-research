...
# /graph-wiki:onboard -> Feature 4).
#
# ## What it does
#
# Fires on the SessionEnd event. Reads `.graph-wiki/active-work.json` (the
# pointer `gw work advance` stamps on every real-pipeline-phase transition —
# see `graph_wiki_core.commands.work._write_active_work_pointer`). If
# present, copies:
...
# ## Escape hatch
#
# Set GRAPH_WIKI_TRANSCRIPT_CAPTURE_GUARD=0 to disable at runtime.

set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
# plugins/graph-wiki/hooks/examples -> repo root is four levels up. Used as
# the uv project so workspace_io/work_io are importable without relying on
# AGENT_RESEARCH_ROOT.
ROOT="${AGENT_RESEARCH_ROOT:-$(cd "${SCRIPT_DIR}/../../../.." && pwd)}"

TRACE_LOG="${GRAPH_WIKI_TRANSCRIPT_CAPTURE_TRACE_LOG:-/tmp/claude-hooks/transcript-capture-trace.log}"
...
if [[ "${GRAPH_WIKI_TRANSCRIPT_CAPTURE_GUARD:-1}" == "0" ]]; then
    trace "skip" "guard=0"
    exit 0
fi
