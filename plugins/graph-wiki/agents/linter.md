...
```bash
python ${CLAUDE_PLUGIN_ROOT}/skills/graph-wiki/scripts/lint_wiki.py --json > /tmp/lint.json
python ${CLAUDE_PLUGIN_ROOT}/skills/graph-wiki/scripts/graph_analyzer.py --json > /tmp/graph.json
```
...
Then log:
```bash
python ${CLAUDE_PLUGIN_ROOT}/skills/graph-wiki/scripts/append_log.py --op lint \
