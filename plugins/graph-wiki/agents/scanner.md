...
Use the package templates from `assets/page-templates/package/`. Each new package gets its own folder with a fully scaffolded set of pages — the overview file (`<workspace>/wiki/packages/<name>/<name>.md`) plus every sub-page template (`api.md`, `context.md`, `patterns.md`, `testing.md`, `work.md`). Routing rules: `<workspace>/wiki/packages/<name>/<name>.md` (cross-domain), `<workspace>/wiki/domains/<d>/packages/<name>/<name>.md` (domain-scoped), `<workspace>/wiki/apps/<name>/<name>.md` for apps, or under a `package-family` row's `vault_dir:` for nested-manifest packages (the scanner emits `wiki_relative_path` per workspace — use it).
