...
- Path to a source file. Either inside `<workspace>/raw/` (staged clip) or repo-relative for an in-repo doc surfaced by `/lattice-wiki:scan` (e.g. `docs/architecture.md`).
...
### 1. Prep
```bash
python ${CLAUDE_PLUGIN_ROOT}/skills/lattice-wiki/scripts/ingest_source.py --source <path> --json
```

(Wiki and repo discovered automatically via `lattice-workspace`. Works for both `raw/` sources and in-repo docs.)
...
### 11. Log
```bash
python ${CLAUDE_PLUGIN_ROOT}/skills/lattice-wiki/scripts/append_log.py --op ingest \
...
- **Md only for now.** PDF/DOCX/HTML auto-discovery is deferred. Direct `/lattice-wiki:ingest <path>` works for any format `ingest_source.py` understands.
