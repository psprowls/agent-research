...
The wiki sits inside a Lattice workspace alongside other workspace-level directories. The LLM must respect the boundaries.

## Layout

The wiki lives at `<workspace>/wiki/`. The workspace is resolved via `lattice-workspace` (defaults to `<repo>/lattice/`; override with `.lattice.local.yaml`'s `lattice-directory` key). The Obsidian vault opens at `<workspace>/`, so `raw/` (immutable sources) and `work/` (work tracker) sit at the workspace root as siblings of `wiki/` — both owned by `lattice-workspace`, not by this plugin.
...
├── raw/                         # IMMUTABLE ingested sources (owned by lattice-workspace)
...
3. **All wiki writes go under `<workspace>/wiki/`.** Work items go to `<workspace>/work/` (owned by `lattice-workspace`). No exceptions.
...
The lifecycle lint rules (`accepted-without-plan`, `stuck-open`, `done-when-missing`, etc.) and the `<workspace>/work/.work-index.json` sidecar generator live in the **`lattice-work`** plugin (a separate install). `lattice-wiki` owns the schema and template only; `lattice-workspace` creates the `work/` directory and `work-tracker` owns the semantics. See `references/lint-workflow.md` for what lives where.
...
- base structural lint (`/lattice-wiki:lint`)
- the work-tracker sidecar (`<workspace>/work/.work-index.json`)
- consumer commands that read the sidecar (e.g. `/lattice-workflows:next`)

Items under `archived/` must already be in a terminal status; the
archive command (`/lattice-work:archive`, owned by
`lattice-work`) enforces this on entry. Restoring is a `git mv`
from `archived/` back to `work/` plus `/lattice-work:regen-index`.

The archive workflow itself is owned by `lattice-work`. See its
`/lattice-work:archive` command and the corresponding spec.
...
In-repo docs (those surfaced by `/lattice-wiki:scan` from a pinned `docs` container) use `source_type: doc`, set `source_path` to the repo-relative path, and record `last_sync_commit` and `last_sync_at`. PDF/DOCX/etc. are deferred — only `.md` is auto-surfaced today.
...
Sections regenerated mechanically on `lattice-wiki:scan`. Marker-bounded so manual content elsewhere on the page survives untouched.
...
- Run `python scripts/update_index.py` after every scan/ingest, OR
