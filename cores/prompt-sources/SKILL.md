---
name: lattice-wiki
description: Use when building or maintaining a persistent wiki alongside any source-code project — single packages, monorepos, or hybrid shapes. Adapts to the repo's folder structure: detects app, package, domain, and docs containers and pins the layout in CLAUDE.md/AGENTS.md. Triggers include "wiki this repo", "document this codebase", "lattice-wiki", "ingest this spec/PR/article into the wiki", or whenever the user wants a compounding, cross-referenced knowledge base alongside source code.
context: fork
version: 0.4.0
author: psprowls
license: MIT
...
Adapts the LLM Wiki pattern ([lattice-wiki](../lattice-wiki/SKILL.md); Karpathy's [gist](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f)) to a source code monorepo. The LLM incrementally builds and maintains a persistent, interlinked markdown vault that documents every package, app, domain, and cross-cutting concept in the repo — plus ingested specs, PR summaries, articles, and design notes.
...
The wiki lives inside the lattice workspace at `<workspace>/wiki/`. The workspace defaults to `<repo>/lattice/` and is discovered automatically via `lattice-workspace` (override with `.lattice.local.yaml`'s `lattice-directory` key). The Obsidian vault opens at `<workspace>/`, so `raw/` (immutable ingested sources) and `work/` (unified work tracker) are siblings of `wiki/` — both owned by `lattice-workspace`, not by this plugin.

```
<repo>/lattice/                 # workspace; Obsidian vault opens here
├── .lattice.yaml               # workspace manifest
├── CLAUDE.md                   # workspace-level schema (owned by lattice-workspace)
├── raw/                        # IMMUTABLE source of truth (owned by lattice-workspace)
...
├── work/                       # unified bugs / tech debt / features / initiatives / spikes (owned by lattice-workspace)
...
2. **Ingest** — read a source (article, spec, PR, transcript, or in-repo doc), discuss takeaways, write a source summary, update 5-15 relevant pages, update index, append to log. In-repo docs (under a `docs` container) are ingested in place — the summary records `source_path` + `last_sync_commit` (when ingested with a clean working tree on main) so /lattice-wiki:lint flags staleness when the file changes. PDF/DOCX support is deferred — see `references/ingest-workflow.md` "Future formats". See `references/ingest-workflow.md`.
...
```bash
# 1. Initialize a wiki in the resolved lattice workspace.
#    Workspace and repo root are discovered via lattice-workspace (walks up from cwd
#    for .git, reads .lattice.local.yaml, defaults to <repo>/lattice).
#    Wiki is created at <workspace>/wiki/ (e.g. lattice/wiki/).
python ${CLAUDE_PLUGIN_ROOT}/skills/lattice-wiki/scripts/init_vault.py \
    --topic "my-repo"
...
/lattice-wiki:ingest raw/specs/auth-migration.md
...
| Command | Purpose |
|---|---|
| `/lattice-wiki:init` | Bootstrap a fresh wiki at `<workspace>/wiki/` (workspace resolved via `lattice-workspace`, defaults to `<repo>/lattice/`) |
| `/lattice-wiki:scan` | Walk the repo, detect packages/apps/workspaces, create/update stub package pages |
| `/lattice-wiki:ingest <path>` | Read a source from `raw/`, discuss, update vault, log it |
| `/lattice-wiki:query <question>` | Search vault, synthesize answer with citations, offer to file back |
| `/lattice-wiki:lint` | Health check — orphans, broken links, stale claims, **code drift** |
| `/lattice-wiki:log` | Show recent log entries (uses unix tools on `log.md`) |

## Sub-agents

| Agent | When dispatched |
|---|---|
| `lattice-wiki:scanner` | Walk the repo, detect packages, propose or update stub package pages |
| `lattice-wiki:ingestor` | Delegated ingest flow — reads source, proposes updates, applies after approval |
| `lattice-wiki:linter` | Runs the health-check workflow (mechanical + semantic + code drift) |
| `lattice-wiki:librarian` | Answers queries using index-first search with citations |

## Python tools (`scripts/`)

Standard library only. Run with `python scripts/<tool>.py --help`.

| Script | Purpose |
|---|---|
| `init_vault.py` | Create folder structure + seed schema files. Wiki path resolved automatically via `lattice-workspace`. |
...
Schema lives in `<workspace>/wiki/CLAUDE.md` (Claude Code) or `<workspace>/wiki/AGENTS.md` (Codex/Cursor/Antigravity/OpenCode/Gemini). The plugin ships both. The scripts run identically everywhere. See `references/cross-tool-setup.md`.
...
| `work` | Unified bug / tech-debt / feature / initiative / spike — replaces issues + roadmap | `<workspace>/work/` (sibling of `<workspace>/wiki/`; owned by `lattice-workspace`) |
...
- **`obsidian-markdown`** — bundled with this plugin. Covers Obsidian-specific syntax (wikilinks, embeds, callouts, properties, comments, highlights). The four sub-agents (`lattice-wiki:scanner`, `lattice-wiki:ingestor`, `lattice-wiki:linter`, `lattice-wiki:librarian`) invoke it whenever they create, edit, or verify a vault page so the output renders correctly in Obsidian.
...
3. **All LLM writes for the wiki go under `<workspace>/wiki/`.** Work items go to `<workspace>/work/` (owned by `lattice-workspace`); ingested sources stay in `<workspace>/raw/` (immutable). No exceptions.
