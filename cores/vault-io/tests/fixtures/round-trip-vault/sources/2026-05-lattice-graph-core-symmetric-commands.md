...
- `packages/lattice-graph-core/src/lattice_graph_core/queries.py` — three new frozen dataclasses (`ImporterRecord`, `ExportRecord`, `ExporterRecord`) and three new query functions (`imported_by`, `exports`, `exported_by`).
- `packages/lattice-graph-core/src/lattice_graph_core/cli/q_imported_by.py` — new CLI module; registers `path`, `--symbol`, `--depth` and dispatches to `queries.imported_by`.
- `packages/lattice-graph-core/src/lattice_graph_core/cli/q_exports.py` — new CLI module; registers `path` and dispatches to `queries.exports`.
- `packages/lattice-graph-core/src/lattice_graph_core/cli/q_exported_by.py` — new CLI module; registers `name` and dispatches to `queries.exported_by`.
- `packages/lattice-graph-core/src/lattice_graph_core/cli/main.py` — three new entries in `_SUBCOMMANDS`.
- `packages/lattice-graph-core/src/lattice_graph_core/cli/_format.py` — extended `render()` dispatch for the three new record types.
