...
Container-aware: reads the wiki's CLAUDE.md for a lattice-wiki:layout block to
...
Discovers repo and wiki locations from the resolved lattice workspace.
...
def discover_docs(repo: Path, wiki: Path, pinned_containers: list[dict]) -> list[dict]:
    ...
    to /lattice-wiki:ingest. PDF/DOCX support is deferred — see
...
def regenerate_dependencies_index(wiki: Path, workspaces: list[dict]) -> Path | None:
    ...
    if not existing:
        existing = (
            ...
            "regenerated by `/lattice-wiki:scan`. Manual notes can sit outside the markers.\n\n"
        )
...
def main():
    ...
    if wiki.exists() and pinned:
        ...
        if any(diff[k] for k in ("new", "missing", "changed")) and not args.json:
            ...
            print("(Re-run /lattice-wiki:init or hand-edit the layout block to update.)")
            print()
    ...
    if doc_candidates:
        ...
        for d in doc_candidates:
            print(f"  ? {d['path']}  (run /lattice-wiki:ingest {d['path']})")
