...
emits a compact deterministic block covering the project's `## Style` rules
and its `## Log format` section.

Pure: no LLM calls, no network, no mutation. Returns the empty string if
...
def render_project_context(wiki_path: Path) -> str:
    ...
    On the first hit, composes a deterministic rendered block covering:
