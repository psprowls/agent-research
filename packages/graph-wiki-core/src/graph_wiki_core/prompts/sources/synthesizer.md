...
```
The `read_file` tool is allow-listed against the repo root and refuses paths inside `.graph-wiki/` (`packages/graph-wiki-core/src/graph_wiki_core/commands/query.py:391`).
```
