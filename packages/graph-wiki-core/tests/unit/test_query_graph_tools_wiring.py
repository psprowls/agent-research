...
def _mock_llm_for(librarian_llm, synth_llm):
    def _llm_for(role: str, *, model_override: str | None = None, backend_override: str | None = None):
        if role == "librarian":
...
async def test_budget_under_proceeds(tmp_path: Path) -> None:
    ...
    extra = [
        ...
        patch("graph_wiki_core.commands.query.build_graph_tools", return_value=[]),
        patch("graph_wiki_core.commands.query.count_tokens", return_value=100),
        patch(
            "graph_wiki_core.commands.query._estimate_tool_schema_tokens",
            return_value=50,
        ),
...
async def test_librarian_loop_iter_cap(tmp_path: Path) -> None:
    ...
    extra = [
        ...
        patch("graph_wiki_core.commands.query.SubagentPool"),
        patch(
            "graph_wiki_core.commands.query._run_code_fallback",
