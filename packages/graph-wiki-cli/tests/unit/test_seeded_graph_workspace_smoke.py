"""Smoke test for the package-local seeded_graph_workspace session fixture.
...
def test_seeded_graph_workspace_yields_path(seeded_graph_workspace: Path) -> None:
    ...
    assert seeded_graph_workspace.exists(), f"seeded_graph_workspace path does not exist: {seeded_graph_workspace}"
...
def test_seeded_graph_workspace_code_db_is_file(seeded_graph_workspace: Path) -> None:
    ...
    assert db.is_file(), f"code.db not found at {db}; update.run may have failed during fixture setup"
