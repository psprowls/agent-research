...
async def test_evalworktree_isolation(fixture_wiki_path: Path) -> None:
    """Two EvalWorktrees opened sequentially have distinct paths."""
    async with EvalWorktree(fixture_wiki_path) as wt1:
        path1 = wt1.path

    async with EvalWorktree(fixture_wiki_path) as wt2:
        path2 = wt2.path
