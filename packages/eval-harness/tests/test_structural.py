...
def test_fixture_wiki_has_pages(fixture_wiki_path: Path) -> None:
    """Fixture wiki must contain more than 3 markdown pages."""
    pages = list(fixture_wiki_path.rglob("*.md"))
    assert len(pages) > 3
...
def test_unresolved_citation(fixture_workspace_path: Path) -> None:
...
def test_all_keys_present(fixture_workspace_path: Path) -> None:
...
def test_no_code_path(fixture_workspace_path: Path) -> None:
...
def test_json_schema_valid(fixture_workspace_path: Path) -> None:
...
def test_invalid_result_type(fixture_workspace_path: Path) -> None:
    """check_structural raises TypeError when result is not a QueryResult (T-4-01)."""
    with pytest.raises(TypeError):
        check_structural({"answer": "bad"}, fixture_workspace_path)  # type: ignore[arg-type]
