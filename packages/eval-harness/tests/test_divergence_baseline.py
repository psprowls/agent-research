...
def test_check_regression_raises_on_hard_increase(tmp_path: Path) -> None:
    ...
    current = _make_results(**{"LIB-001-wikilink-resolves": {"runs": 5, "failures": 3, "accepted_failures": []}})
...
def test_check_regression_does_not_raise_for_soft(tmp_path: Path) -> None:
    ...
    current = _make_results(**{"LIB-004-code-path-format": {"runs": 5, "failures": 3, "accepted_failures": []}})
    # Must not raise
    check_regression("librarian", current, baseline)
...
def test_check_regression_does_not_raise_for_judge(tmp_path: Path) -> None:
    ...
    current = _make_results(**{"LIB-JUDGE": {"runs": 5, "failures": 4, "accepted_failures": []}})
    # Must not raise
    check_regression("librarian", current, baseline)
...
def test_check_regression_passes_when_equal(tmp_path: Path) -> None:
    ...
    results = _make_results(**{"LIB-001-wikilink-resolves": {"runs": 5, "failures": 2, "accepted_failures": []}})
    write_baseline("librarian", tmp_path, results, "abc")
    baseline = load_baseline("librarian", tmp_path)

    # Same failures — equal, should not raise
    current = _make_results(**{"LIB-001-wikilink-resolves": {"runs": 5, "failures": 2, "accepted_failures": []}})
    check_regression("librarian", current, baseline)
...
def test_check_regression_passes_when_decreased(tmp_path: Path) -> None:
    ...
    results = _make_results(**{"LIB-001-wikilink-resolves": {"runs": 5, "failures": 5, "accepted_failures": []}})
    write_baseline("librarian", tmp_path, results, "abc")
    baseline = load_baseline("librarian", tmp_path)

    # Fewer failures — improvement, should not raise
    current = _make_results(**{"LIB-001-wikilink-resolves": {"runs": 5, "failures": 1, "accepted_failures": []}})
    check_regression("librarian", current, baseline)
...
def test_accept_baseline_flag_rewrites_file(tmp_path: Path) -> None:
    ...
    """
    first_results = _make_results(**{"LIB-001-wikilink-resolves": {"runs": 3, "failures": 1, "accepted_failures": []}})
    ...
    second_results = _make_results(**{"LIB-001-wikilink-resolves": {"runs": 5, "failures": 3, "accepted_failures": []}})
