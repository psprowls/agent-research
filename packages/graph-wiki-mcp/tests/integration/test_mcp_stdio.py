...
def _run_server(payload_objs: list[dict]) -> tuple[str, str]:
    ...
    proc = subprocess.Popen(
        ["uv", "run", "--package", "graph-wiki-mcp", "graph-wiki-mcp"],
    ...
    try:
        ...
        pytest.fail(
            "MCP server did not respond within 15s.\n"
            f"stdout: {stdout_bytes.decode()[:500]}\n"
...
def test_mcp_stdout_is_valid_jsonrpc():
    ...
    for line in lines:
        ...
        assert obj.get("jsonrpc") == "2.0", f"Line is JSON but not a JSON-RPC 2.0 frame: {line!r}"
        assert "id" in obj or "method" in obj, f"Line is JSON-RPC but lacks response/request shape: {line!r}"
...
def test_wiki_query_in_tools_list() -> None:
    ...
    triggers import of graph_wiki_core.commands.query, which imports bm25s,
