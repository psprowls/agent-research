...
- The tool is allow-listed: it refuses paths outside the repo root, inside `.code-wiki/`, or non-regular-files. Refusals come back as a string starting with `ERROR:`.
...
4. **Never read `.code-wiki/`.** Those are vault metadata, not source. The tool refuses; honor the refusal.
...
- Trying to read paths inside `.code-wiki/` → tool refuses; do not retry with similar paths.
