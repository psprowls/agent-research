...
| *(none)* | `work/` — unified bug / tech-debt / feature / initiative / spike namespace |
...
Consequence: after major refactors, run `/lattice-wiki:scan` and `/lattice-wiki:lint` to surface the drift. Don't trust the wiki's claims about code without spot-checking.
