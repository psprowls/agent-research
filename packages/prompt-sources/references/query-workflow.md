...
The flow the LLM follows when the user runs `/lattice-wiki:query <question>` or dispatches the `lattice-wiki:librarian` sub-agent.
...
```bash
python scripts/wiki_search.py --query "<terms>" --limit 5
```
...
- Append to `log.md`:
  ```bash
  python scripts/append_log.py --op create \
...
- ❌ Grep the entire repo on every query → use the index, then drill into the wiki, then to code only if needed
- ❌ Answer without citations → every claim must link to a wiki page or code path
- ❌ Create a new page for a trivial one-off question → only file back substantive answers worth keeping
- ❌ Invent content not in the wiki or code → if you don't know, say so and suggest a source to ingest or a concept page to create
