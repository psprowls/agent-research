...
def test_entities_dir_bootstrapped_after_init_wiki(
    ...
    """URI-04 / D-14, D-15: init_wiki creates wiki/entities/_index.md with sentinel.
    ...
    only the directory-creation + sentinel-write path.
    """
    import re
    ...
    entities_index = entities_dir / "_index.md"
    assert entities_dir.is_dir(), f"entities/ dir not created: {entities_dir}"
    assert entities_index.is_file(), f"_index.md not created: {entities_index}"
    body = entities_index.read_text(encoding="utf-8")
    assert re.match(r"<!--\s*generated by graph-wiki-agent scan", body), (
        f"sentinel comment missing or wrong shape: {body!r}"
    )
    assert "entities/_index.md" in result["installed_files"], (
        f"installed_files missing entities/_index.md: {result['installed_files']}"
    )
