...
Two-pass regex sweep per ADR-0015: [[../work/...]] → [[work/...]] and bare [[<container>/...]] → [[<container>/...]] across every concept, ADR, package/plugin, and sources page (~31 concept pages bumped). Stale-layout flags added to concepts/lattice-vault-terminology.md and concepts/per-repo-layout.md.
...
Renamed the Python package directory and module to `packages/lattice-curator-core` (`lattice_curator_core`). Plugin-facing identifiers (plugin name, MCP server, `.lattice-curator.json`, `~/.cache/lattice-curator/`) unchanged. Touched: [[packages/lattice-curator-core/lattice-curator-core]], [[index]], plus wikilink/path sweep across concepts, ADRs, and plugin pages.
...
Wrote [[sources/2026-05-execution-skills-comparison]] + new concept [[concepts/execution-skills-comparison]]; updated [[plugins/lattice-workflows/lattice-workflows]], [[plugins/lattice-workflows/api]], [[plugins/lattice-workflows/patterns]], [[concepts/subagent-vs-teammate]]; regenerated index.

## [2026-05-10] ingest | Model Selection Guidelines

Added [[sources/2026-05-model-selection-guidelines]] + new concept [[concepts/model-selection-per-skill]]; updated [[plugins/lattice-workflows/patterns]] (new pattern bullet), [[plugins/lattice-workflows/api]] (model: frontmatter note), and [[plugins/lattice-workflows/lattice-workflows]] (sources++). Flagged vault-vs-code contradiction: code-reviewer is an agent (not a skill) and code-scanner does not exist in the codebase.
