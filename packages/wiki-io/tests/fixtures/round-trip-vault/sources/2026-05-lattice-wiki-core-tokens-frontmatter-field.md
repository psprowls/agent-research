...
6. **All 19 page templates seed `tokens: 0`.** 12 top-level (`adr`, `app`, `architecture`, `concept`, `concept-pattern`, `dependency`, `domain`, `index`, `package-family`, `package`, `source`, `work`) + 7 sub-templates (`domain/{overview,details}`, `package/{overview,api,context,patterns,work}`). Placeholder is overwritten on first run.
