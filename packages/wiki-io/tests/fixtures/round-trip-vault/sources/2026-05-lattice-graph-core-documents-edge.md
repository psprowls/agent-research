...
- `packages/lattice-graph-core/src/graph_io/sync_wiki.py` — new module: workspace resolution, package-node load, three-convention path resolver, glob walk for domain-scoped packages, upsert + cleanup, drift-report printer.
- `packages/lattice-graph-core/src/graph_io/cli/ops_sync_wiki.py` — new thin CLI dispatch following the `ops_update.py` pattern.
- `packages/lattice-graph-core/src/graph_io/cli/main.py` — register `"sync-wiki": ops_sync_wiki`.
