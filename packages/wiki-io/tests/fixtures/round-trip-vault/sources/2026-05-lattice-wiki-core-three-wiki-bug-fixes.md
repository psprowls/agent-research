...
4. **Tests added.** `test_update_index_categories.py` (new), `test_lint_json_drift.py` (new), and one assertion in `test_init_vault_dirs.py`. All listed in the v0.3.0 entry of [[packages/lattice-wiki-core/lattice-wiki-core#File map]] under `tests/`.
...
- ==Wiki drift surfaced during ingest:== `[[packages/lattice-wiki-core/api]]` listed `SUBPAGE_STEMS = {"api", "patterns", "issues", "context", "flows"}` — pre-fix value. Updated as part of this ingest to add `"work"` so the wiki agrees with the code.

## Touches

- [[packages/lattice-wiki-core/lattice-wiki-core]]
- [[packages/lattice-wiki-core/api]]
- [[packages/lattice-wiki-core/patterns]]
- [[packages/lattice-wiki-core/work]]
- [[packages/lattice-wiki-core/context]]
