...
- **Plugin stays thin.** No curation logic in the plugin — every interesting decision lives in [[packages/lattice-curator-core/lattice-curator-core]]. Mirrors the [[packages/lattice-source-parser/lattice-source-parser]] precedent.
...
- **Naming convention.** Hook script files use underscores (`curator_fire.py`, `stage_tracker.py`); hook registration arguments in `hooks.json` use dashes (`curator-fire`, `stage-tracker`). This mismatch is a known bug — see [[plugins/lattice-curator/work]].
