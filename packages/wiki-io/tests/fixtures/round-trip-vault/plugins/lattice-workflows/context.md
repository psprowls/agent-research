...
- [[concepts/lattice-naming-convention]]
- [[concepts/lattice-workflows-consumption-seam]] — the five integration patterns and the consumer-not-writer principle (note: workflow now also writes the workspace-sibling `specs/` and `plans/` trees per [[adrs/0013-plans-and-specs-in-lattice-workspace]])
- [[concepts/lattice-cross-plugin-contract]] — env-var discovery and subprocess invocation used by the v1 cross-plugin scripts
- [[concepts/lattice-workflows-observability-gate]] — `LATTICE_WORKFLOWS_OBSERVABILITY` opt-out category gate + fail-open invariant for hooks under `plugins/lattice-workflows/hooks/`
- [[concepts/superpowers-fork-vs-upstream]] — lineage diagram (obra → pcvelz → lattice-workflows), comparison table, and decision rubric for which upstream to track

## Decisions

- [[adrs/0003-observability-as-category-gate]] — single opt-out env var `LATTICE_WORKFLOWS_OBSERVABILITY` gates every observability hook in this plugin; fail-open invariant
- [[adrs/0013-plans-and-specs-in-lattice-workspace]] — relocate brainstorming specs and implementation plans from `docs/lattice-workflows/{specs,plans}/` to `<workspace>/{specs,plans}/`; prefix filenames by package/plugin; hard-fail on missing `.lattice.yaml`
- [[adrs/0016-track-pcvelz-superpowers-fork]] — track `pcvelz/superpowers` as upstream rather than `obra/superpowers`; accepts CC-only portability in exchange for native `TaskCreate`/`blockedBy`/`.tasks.json` and plan-mode hard-gate

## Sources

- [[sources/2026-05-plans-specs-path-redesign]] — design behind ADR-0013; specifies workspace-resolution helper, prefix inference rules, error-on-missing-manifest behavior, and the brainstorming + writing-plans skill files that change
...
- [[sources/2026-05-superpower-fork-selection]] — comparison of `pcvelz/superpowers` vs `obra/superpowers`; backs [[concepts/superpowers-fork-vs-upstream]] and [[adrs/0016-track-pcvelz-superpowers-fork]]
...
- [[plugins/lattice-curator/lattice-curator]] — the curator's `PreToolUse:Skill` hook (`stage-tracker.mjs`) records whenever any Skill tool is invoked, including `lattice-workflows:*` skills. The recorded `lastSkill` drives stage routing in the curator's gate. No contract change to lattice-workflows — purely passive observation.
