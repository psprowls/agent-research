...
- **Shared `lib/` for cross-plugin invocation** (per [[concepts/lattice-cross-plugin-contract]]) — `lib/work_index.py` parses the work-tracker sidecar consumed by `:next` and `:status`; `lib/lattice_invoke.py` resolves `${LATTICE_WIKI_ROOT}` / `${LATTICE_WORK_ROOT}` and shells out to sibling-plugin scripts. Pattern: subprocess-not-import, env-var discovery, fail-soft on missing plugins.
