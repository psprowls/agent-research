...
- [[concepts/code-wiki-pattern]]
- [[concepts/lattice-naming-convention]]
- [[concepts/per-repo-layout]]
- [[concepts/lattice-work-namespace-schema]] — the unified `work/` schema, frontmatter, lifecycle, sidecar contract
- [[concepts/lattice-page-body-table-conventions]] — `## Plan` / `## Endpoints` / `## Fields` table grammar
- [[concepts/lattice-dependencies-tiering]] — `dependencies/index.md` regen + opt-in detail + `kind` discriminator
- [[concepts/lattice-vault-terminology]] — canonical glossary; pending renames (`domain`, `container` umbrella)
- wiki-cites-graph-not-duplicates — consumer-side relationship with [[plugins/lattice-graph/lattice-graph]]: graph-aware lint with filesystem fallback; wiki cites graph rather than duplicating it

## Decisions

- [[adrs/0001-sqlite-primary-store-for-code-graph]] — renamed from `llm-code-wiki`
- [[adrs/0002-explicit-graph-update-lifecycle]] — graph stays a separate plugin; this plugin is consumer of its surface
...
- [[adrs/0005-sourcetree-sole-domain-model]] — owns `<repo>/wiki/`
...
- 2026-05-archive-resolved-work-items — establishes the `work/archived/` sub-namespace owned by [[plugins/lattice-work/lattice-work]]; this plugin's `lint_wiki.py` excludes that path from its work-namespace lint walks per the archive contract
...
- [[plugins/lattice-work/lattice-work]] — hard one-way dep; reads `<vault>/work/*.md`, writes `<vault>/work-index.json`. Schema and migration script live here.
- [[plugins/lattice-curator/lattice-curator]] — the curator's wiki `Source` adapter walks `<vaultDir>/**/*.md` and turns frontmatter into a flat catalog feeding the [[concepts/two-pass-context-curation|two-pass retriever]].

## Related dependencies

- [[packages/lattice-wiki-core/lattice-wiki-core]] — shared pure-Python IO + analysis library backing this plugin (template assets, lint, scan, ingest, layout)
