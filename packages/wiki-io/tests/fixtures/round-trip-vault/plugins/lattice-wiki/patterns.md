...
- **Three-layer architecture** — `<repo>/` is the source of truth; `<wiki>/raw/` holds immutable ingested sources; `<wiki>/<vault>/` is the LLM-curated Obsidian vault. The plugin only writes to the vault. See [[concepts/code-wiki-pattern]] and [[concepts/per-repo-layout]].
...
- **Pure-stdlib invariant** — every script under `scripts/` runs on the Python standard library. The pure-stdlib invariant is local to this plugin and preserved even though [[plugins/lattice-graph/lattice-graph]] takes a binary tree-sitter dep.
...
- **`work/archived/` carve-out** — `lint_wiki.py` walks `<vault>/work/*.md` for the work-namespace lint group but excludes the `<vault>/work/archived/` sub-namespace per the [[plugins/lattice-work/lattice-work]] archive contract. Archived work items are owned and rendered by `lattice-work:archive`; the wiki's structural lint must not flag them as orphans or miswired.
