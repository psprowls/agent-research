...
- **SQLite primary store** — nodes/edges/metadata at `<repo>/.lattice/graph/code.db`. Two-table + metadata schema with recursive CTEs for bounded traversal. See [[concepts/code-graph-schema]] and [[adrs/0001-sqlite-primary-store-for-code-graph]].
- **`lattice-source-parser` as parsing substrate** — this library does not parse source directly; it calls `parse_file` + `to_graph_records` from [[packages/lattice-source-parser/lattice-source-parser]] and upserts the resulting `GraphRecords`. The parser package never sees integer ids — tuples in, ids out at the `upsert.py` boundary.
- **Single-writer DB** — only `cg update` writes to `code.db`; all queries open with `mode=ro` URI. SQLite write-lock structurally prevents concurrent writers; second `cg update` waits 30 s then exits `6` (`UPDATE_IN_PROGRESS`). Per [[adrs/0008-single-writer-code-db]].
...
- **Stable exit codes** — `exit_codes.py` declares all exit codes from v1 forward; script consumers can rely on these. See [[packages/lattice-graph-core/api]] for the full table.
- **CLI as adapter** — `cli/` is a thin adapter over the query and operational modules. Query logic lives in `queries.py`; ops in `update.py`. A future MCP adapter shares the same library, so the boundary stabilizes by use before MCP lands. Per [[adrs/0007-cli-first-code-graph]].
...
- **uv workspace member** under the repo-root workspace coordinator (single `uv.lock`, `uv sync` from repo root). Per [[adrs/0009-uv-ruff-python-tooling]].
