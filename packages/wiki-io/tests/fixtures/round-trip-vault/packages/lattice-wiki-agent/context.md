...
`Config.backend_for(command)` (`config.py:35`) lets each operation independently choose between `"claude"` (mechanical only — scan/lint/log/init can run with no LLM at all) and `"bedrock"` (build a `ChatBedrockConverse` and route the LLM steps through it). `ingest` and `query` have no mechanical-only mode, so the CLI fails fast if `bedrock` isn't selected for those commands. See [[packages/lattice-wiki-agent/api]] for the gating rules.
...
- No ADR yet for "Bedrock as the headless LLM backend". The choice mirrors the [[concepts/lattice-naming-convention]] and the existing `lattice-curator` Bedrock factory.
...
No formal domain page yet; conceptually part of the wiki maintenance toolchain alongside [[packages/lattice-wiki-core/lattice-wiki-core]].

## Used by

- [[plugins/lattice-wiki/lattice-wiki]] — sibling automation surface; the plugin drives the same operations interactively while the agent drives them headlessly.
...
- `click>=8.1` — CLI.
- [[packages/lattice-wiki-core/lattice-wiki-core]] — the library every agent delegates to for deterministic work.
- [[concepts/explicit-not-magic-update-lifecycle]] — the lifecycle the agent automates.
