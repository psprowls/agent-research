...
- [[work/2026-05-09-fix-vault-rooted-wikilinks]] / [[work/2026-05-09-adjust-linter-for-work-sibling-to-vault]] — **bug, open.** Linter walks `<workspace>/wiki/` only and 22+ wiki pages use `[[../work/...]]`; both downstream of [[adrs/0011-single-workspace-root]]. Resolution path approved in [[sources/2026-05-workspace-relative-wikilinks-linter-and-content-rewrite]] — switch the walk to `<workspace>/`, key pages workspace-relative, gate structural checks via `linted` / `is_work` flags, and rewrite content with a two-pass regex. Decision: [[adrs/0015-workspace-root-wikilink-form]].

### Resolved (bundled in [[sources/2026-05-lattice-wiki-core-three-wiki-bug-fixes]])
...
- **No version field surfaced.** `pyproject.toml:7` pins `version = "1.0.0"`, but nothing in the package surfaces it (no `__version__`, no `--version` flag on any CLI). Will matter if/when installed via PyPI or pinned by [[packages/lattice-wiki-agent/lattice-wiki-agent]].
...
- **Should `ingest_work_item.py` live in this package?** It's deeply cross-plugin (used by `lattice-workflows` and `lattice-work`) and changes to work-page schema force `lattice-wiki-core` releases. Consider whether it belongs in [[packages/lattice-wiki-agent/lattice-wiki-agent]] or a new `lattice-work-core` package.
